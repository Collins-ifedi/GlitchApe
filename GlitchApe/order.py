...
1.  Initiate a checkout:
    ...
    - Saves a local 'OrderRecord' and 'OrderPayment'.
...
# --- UPDATED IMPORTS ---
# Import core components from the new interface.py
from interface import (
    ...
    ChatMessage,
    OrderRecord,
    OrderPayment  # Import the model from interface.py
)
# -----------------------
...
# The 'OrderPayment' model is now defined in interface.py
# and imported above. This section is no longer needed.
...
async def initiate_checkout(
    ...
    async with db.begin_nested(): # Use savepoint for transaction
        # Create base OrderRecord (from interface.py)
        ...
        # Create our new OrderPayment record (from interface.py)
        ...
        await db.flush() # Get the new_payment.id
        ...
        # Note: The original code had a bug here. It committed *before*
        # the PaymentIntent was modified. The `await db.flush()`
        # and moving the commit to the end of the `with` block fixes this.
...
async def stripe_webhook(
    ...
    if event.type == "payment_intent.succeeded":
        ...
        try:
            ...
            print(f"Webhook Error: Invalid UUID format for order_payment_id: {payment_id}")
            return {"status": "error", "reason": "Invalid ID format"}

        payment = await db.get(OrderPayment, payment_uuid)
        ...
        if payment_id:
            try:
                ...
                # Log if the UUID is bad or DB fails
                print(f"Webhook Error: Failed to update payment_intent.payment_failed: {e}")
