...
# Imports for new routes are already present
...
try:
    ...
    # Set to None to force startup failure
    OrderPayment = None
    Base = None
    engine = None
...
# --- Sanity checks for other modules' keys (still good practice) ---
if not os.getenv("AI_API_KEY"):
    raise RuntimeError("AI_API_KEY env var required for glitchape_central_handler.py")
if not os.getenv("PRINTFUL_API_KEY"):
    raise RuntimeError("PRINTFUL_API_KEY env var required for glitchape_central_handler.py")
if not os.getenv("STRIPE_SECRET_KEY"):
    raise RuntimeError("STRIPE_SECRET_KEY env var required for glitchape_central_handler.py")
if not os.getenv("STRIPE_WEBHOOK_SECRET"):
    raise RuntimeError("STRIPE_WEBHOOK_SECRET env var required for glitchape_central_handler.py")
...
async def startup():
    ...
    # --- UPDATED ROUTER MOUNTING ---
    # Attach the single Central Brain router
    if central_router:
        # The endpoints in central_router already have prefixes like /ai/ and /orders/
        # So we mount it directly at /api
        app.include_router(central_router, prefix="/api")
        log.info("Central Brain router (glitchape_central_handler.py) loaded successfully at /api.")
    else:
        log.error("Central Brain router not found. AI and Order endpoints will be unavailable.")
    # ---------------------------------
...
# -----------------------
# Auth endpoints (Remain in server.py)
# -----------------------
...
# -----------------------
# Chat session endpoints (Remain in server.py)
# -----------------------
...
async def serve_frontend_spa(path: str):
    ...
    This is CRITICAL for paths like /verify?token=... when clicked from an email.
    """
    # List of known frontend routes that should load the index.html
    frontend_routes = ("verify", "reset-password", "auth-success", "auth-error")
    
    # Check if the path starts with any of the known frontend routes
    # This correctly handles paths with query parameters, e.g., "verify?token=..."
    ...
    # Redirect unknown paths back to the root to restart the SPA
