...
async def startup_event():
    """
    Asynchronous event that runs on application startup.
    - Connects to the database.
    - Creates database tables based on the models.
    """
    logger.info("🚀 Starting up application...")
    async with engine.begin() as conn:
        # NOTE: In a production environment, you should use a database migration
        # tool like Alembic to manage your schema instead of `create_all`.
...
async def shutdown_event():
    """
    Asynchronous event that runs on application shutdown.
