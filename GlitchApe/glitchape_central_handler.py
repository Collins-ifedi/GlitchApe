...
from sqlalchemy import select, JSON # <-- Keep JSON for draft details type hinting

from PIL import Image

# Import core components from interface.py
# Ensure interface.py has the draft_order_details column (JSON type) in ChatSession
try:
...
VISION_MODEL = "gemini-1.5-flash" # Use 1.5 Flash as requested previously
GEMINI_TIMEOUT = 90 # Seconds

# Global Gemini Client (Asynchronous preferred if available, but sync works with thread)
...
GLITCHAPE_PUBLIC_URL = os.getenv("GLITCHAPE_PUBLIC_URL", "http://localhost:8000").rstrip('/')
...
# Values are Printful Product IDs from their Catalog API (e.g., GET /products)
PRODUCT_TYPE_TO_PRINTFUL_ID = {
    "t-shirt": 71, # Example: Bella + Canvas 3001
    "tshirt": 71,
    "tee": 71,
    "hoodie": 162, # Example: Gildan 18500
    "sweatshirt": 162, # Map synonyms
    # Add more mappings: mugs, posters, hats etc.
    "poster": 1, # Example: Enhanced Matte Paper Poster
    "mug": 19, # Example: White glossy mug
}
...
# --- File Storage ---
UPLOAD_DIR = "temp_images"; os.makedirs(UPLOAD_DIR, exist_ok=True)
MAX_FILE_SIZE_MB = 10; MAX_FILE_SIZE_BYTES = MAX_FILE_SIZE_MB * 1024 * 1024
MIME_TYPES = {"png": "image/png", "jpg": "image/jpeg", "jpeg": "image/jpeg", "webp": "image/webp"}

# --- Country/State Codes (Cache - Populate on startup or first request) ---
VALID_COUNTRIES: Dict[str, str] = {} # Code -> Name
COUNTRIES_NEEDING_STATES = {"US", "CA", "AU", "JP"} # Codes based on Printful Docs
VALID_STATES: Dict[str, Dict[str, str]] = {} # Country Code -> {State Code -> State Name}

# ===================================================================
# PYDANTIC SCHEMAS (Mostly Unchanged)
# ===================================================================
...
class CheckoutRequest(BaseModel): session_id: str # Only session_id needed
...
async def _find_printful_variant_id(product_type: str, size: str, color: str) -> Optional[int]:
    ...
    if variants is None: # Indicates API error during fetch
    ...
    # Basic fuzzy matching (e.g., ignore common words like 'heather')
...
async def _populate_location_data():
    ...
    if VALID_COUNTRIES: return # Already populated
...
def _validate_country_code(code: str) -> Optional[str]:
    ...
    if not VALID_COUNTRIES: # Attempt to populate if empty
         log.warning("Country list not populated yet. Validation might be incomplete.")
         # In a production scenario, might trigger async population here or fail
    
    code_upper = code.strip().upper()
    if code_upper in VALID_COUNTRIES:
        return code_upper
    # Try finding by name (basic)
...
def _validate_state_code(country_code: str, state_input: Optional[str]) -> Optional[str]:
    ...
    if country_code not in COUNTRIES_NEEDING_STATES:
        return state_input # Not required or not validated here

    if not state_input: # Required but missing
    ...
    if not VALID_STATES.get(country_code):
         ...
         return state_input.strip().upper() # Return upper-cased input as best guess
        ...
        for filename in os.listdir(UPLOAD_DIR):
            ...
            if os.path.isfile(file_path):
                try:
                    ...
                    if now - file_time > timedelta(hours=48): os.remove(file_path); log.info(f"Cleaned up expired image: {filename}")
                except OSError as e: log.warning(f"Failed to cleanup image {filename}: {e}")
    except Exception as e: log.error(f"Error during image cleanup: {e}")


async def _save_temp_image(file: UploadFile) -> str:
    """Saves uploaded image, validates, returns full local path."""
    # (Implementation mostly unchanged)
    if not file.filename: raise HTTPException(status_code=400, detail="No file name provided.")
    original_filename = file.filename; ext = original_filename.split(".")[-1].lower()
    if ext not in MIME_TYPES: ext = "png" # Default

    if not file.content_type or not file.content_type.startswith("image/"):
        log.warning(f"File {original_filename} header MIME type {file.content_type} suspect.")
        if ext not in MIME_TYPES: raise HTTPException(status_code=400, detail="Invalid file type.")

    file_id = f"{uuid.uuid4()}.{ext}"; file_path = os.path.join(UPLOAD_DIR, file_id)

    try:
        file_content = await file.read()
        if len(file_content) > MAX_FILE_SIZE_BYTES: raise HTTPException(status_code=413, detail=f"File too large (max {MAX_FILE_SIZE_MB}MB).")
        with open(file_path, "wb") as f: f.write(file_content)
        try:
            with open(file_path, "rb") as f: img = Image.open(f); img.verify()
        except Exception as img_err:
            log.error(f"Image integrity check failed for {file_path}: {img_err}")
            if os.path.exists(file_path): os.remove(file_path)
            raise HTTPException(status_code=400, detail="Uploaded file is corrupted/invalid image.")
        return file_path
...
async def _analyze_image_with_vision_model(local_image_path: str, user_prompt: str) -> str:
    """Uses Gemini SDK to describe image content."""
    # (Updated for better error handling)
    global gemini_client
    if not gemini_client: return "ERROR: Gemini Client not initialized."

    log.info(f"Calling Gemini ({VISION_MODEL}) via SDK for analysis on {local_image_path}...")
    image_part = None
    try:
        # Upload file to Gemini service (required for multimodal input with SDK)
        image_part = genai.upload_file(path=local_image_path)
        while image_part.state.name == "PROCESSING":
            await asyncio.sleep(2) # Short sleep while processing
            image_part = genai.get_file(image_part.name)
        if image_part.state.name == "FAILED":
            raise ValueError(f"Gemini image upload failed: {image_part.name}")
        ...
        # Use asyncio.to_thread to wrap synchronous generate_content call
        def sync_generate():
        ...
        if not analysis_text: log.warning(f"Gemini returned empty analysis for {local_image_path}."); return "Vision model returned empty analysis."
        log.info(f"Gemini analysis successful for {local_image_path}.")
        return analysis_text

    except google_exceptions.GoogleAPIError as e:
        log.error(f"Gemini API Error: {e}", exc_info=True)
        return f"ERROR: Gemini API Error - {e.message[:100]}"
    except ValueError as e: # Handle upload failure
         log.error(f"Gemini Value Error (likely upload): {e}", exc_info=True)
        ...
        # Ensure cleanup of the uploaded file on Gemini side
        if image_part and hasattr(image_part, 'name'):
            try: genai.delete_file(image_part.name)
            except Exception as del_e: log.warning(f"Could not cleanup Gemini file {image_part.name}: {del_e}")


async def _generate_design_artwork(prompt: str) -> str:
    """Generates standalone artwork decal (PNG transparent bg)."""
    ...
    try:
        ...
        try: img = Image.open(BytesIO(image_data)) # Basic validation
        except Exception: log.error("HF AI returned invalid image data."); raise HTTPException(500, "AI returned invalid image data.")
        file_id = f"{uuid.uuid4()}.png"; file_path = os.path.join(UPLOAD_DIR, file_id)
        with open(file_path, "wb") as f: f.write(image_data)
        log.info(f"Generated artwork saved to {file_path}")
        return file_path
    except httpx.HTTPStatusError as e:
...
async def _generate_mockup_preview(design_description: str, clothing_type: str, view: str = "front") -> str:
    """Generates user-facing mockup preview image."""
    # (Added robust error handling)
    ...
    try:
        ...
        image_data = response.content
        try: img = Image.open(BytesIO(image_data)) # Basic validation
        except Exception: log.error("HF AI returned invalid mockup data."); raise HTTPException(500, "AI returned invalid mockup data.")
        file_id = f"{uuid.uuid4()}_mockup.png"; file_path = os.path.join(UPLOAD_DIR, file_id)
        with open(file_path, "wb") as f: f.write(image_data)
        log.info(f"Generated mockup saved to {file_path}")
        return file_path
    except httpx.HTTPStatusError as e:
...
async def _upload_to_printful(image_msg: ChatMessage, headers: Dict[str, str]) -> Dict[str, Any]:
    """Uploads single local design artwork file to Printful."""
    # (Added robust error handling)
    relative_url = image_msg.image_url
    if not relative_url: raise HTTPException(500, f"DB record missing URL for image {image_msg.id}.")
    filename = os.path.basename(relative_url); local_path = os.path.join(UPLOAD_DIR, filename)
    if not os.path.exists(local_path): raise HTTPException(404, f"Image file not found on server: {local_path}")
    view_name = "main_design_decal"
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            with open(local_path, "rb") as f:
                ext = filename.split(".")[-1].lower(); mime_type = MIME_TYPES.get(ext, "image/png")
                files = {"file": (filename, f, mime_type)}
                resp = await client.post(f"{PRINTFUL_API_URL}/files", headers=headers, files=files)
                resp.raise_for_status() # Check for 4xx/5xx
        data = resp.json().get("result")
        if not data or not data.get("id"): log.error(f"Printful upload no file ID. Resp: {resp.text}"); raise HTTPException(502, "Printful API did not return File ID.")
        log.info(f"Uploaded {filename} to Printful, ID: {data['id']}")
        ...
        log.error(f"Unexpected error uploading {filename} to Printful: {e}", exc_info=True)
...
async def _get_printful_costs(item: OrderItem, recipient: Recipient, headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    # (Implementation unchanged, relies on upstream validation)
    try:
        async with httpx.AsyncClient(timeout=PRINTFUL_TIMEOUT) as client:
            ...
            if not rate_data: log.warning(f"No shipping rates returned for {item.variant_id} to {recipient.country_code}. Using fallback."); shipping_cost = 5.0 # Fallback
            else: shipping_cost = float(rate_data[0].get("rate", 0)) # Use first available rate
...
async def _submit_order_to_printful(payment: OrderPayment, order: OrderRecord, headers: Dict[str, str], db: AsyncSession):
    ...
    try:
        ...
        printful_files = [{"id": artwork_printful_id, "type": "front"}] # Assume front placement
        ...
        log.error(err_msg, exc_info=not isinstance(e, ValueError)) # Less verbose for expected value errors
        payment.status = "error"; payment.error_message = err_msg; await db.commit() # Save error state


def _parse_state_zip(state_zip_input: str) -> Dict[str, Optional[str]]:
    """Tries to extract state code and zip code."""
    # (Implementation unchanged - basic parsing)
    ...
    if len(parts) >= 2:
        ...
        else: state_code = part1.upper(); zip_code = " ".join(parts[1:]) # Best guess
    elif len(parts) == 1:
...
class GlitchApeCentralHandler:
    ...
    def __init__(self, db: AsyncSession, user: Optional[User] = None):
        self.db = db
        self.user = user
        # Ensure location data is loaded (can be background task on startup too)
    ...
    async def _call_llama_brain(self, prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            else: log.error(f"LLaMA failed to include JSON object. Raw: {llama_response_content}"); json_str = clean_content # Try parsing anyway as fallback
        ...
        except (IndexError, KeyError, Exception) as e: # Catch issues with response structure or other errors
    ...
    # --- MAIN CHAT HANDLER (with enhanced validation) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        # Ensure draft_details is a mutable dict
        draft_details = session.draft_order_details if isinstance(session.draft_order_details, dict) else {}

        # --- Local variables ---
        local_image_path: Optional[str] = None; image_analysis_text: Optional[str] = None
        analysis_error_message: Optional[str] = None; db_image_url: Optional[str] = None
        ai_text_response = "Processing your request..." # Default response
        next_state = current_state
        image_previews: List[ImagePreview] = []
        ask_user_to_clarify = False # Flag if validation fails
        clarification_request = "" # Specific clarification message

        # 1. Handle Upload & Vision (Only in designing state)
        if current_state == "designing" and uploaded_image and uploaded_image.filename:
            try:
                local_image_path = await _save_temp_image(uploaded_image)
                filename = os.path.basename(local_image_path); db_image_url = f"/api/ai/image/{filename}"
                log.info(f"User {self.user.id} uploaded '{filename}'. Analyzing...")
                image_analysis_text = await _analyze_image_with_vision_model(local_image_path, prompt)
            ...
            except HTTPException as e: raise e # Propagate HTTP errors from saving/validation
        ...
        elif db_image_url: final_prompt_for_llama = f"[IMAGE UPLOADED: {db_image_url} - Analysis Skipped] User request: {prompt}"
        ...
        except HTTPException as e: # Handle LLaMA call errors specifically
        ...
        ai_text_response = action_json.get("response_text", "Got it, processing...") # LLaMA's intended response

        try:
            # --- Designing State ---
            if current_state == "designing":
                if intent == "design_request" or (intent == "image_analysis" and db_image_url):
                    # (Design/Mockup generation logic - unchanged, assuming _generate funcs handle errors)
                    ...
                    if db_image_url: # User uploaded
                        ...
                        artwork_path = await _generate_design_artwork(design_prompt)
                        artwork_url = f"/api/ai/image/{os.path.basename(artwork_path)}"
                    ...
                    if artwork_url and mockup_prompt_source:
                        ...
                        self.db.add(ai_artwork_msg); draft_details['artwork_image_url'] = artwork_url # Store ref

                        mockup_path = await _generate_mockup_preview(mockup_prompt_source, clothing_type)
                        mockup_url = f"/api/ai/image/{os.path.basename(mockup_path)}"
                        ...
                        # Use LLaMA's generated response_text here if available and relevant, otherwise default
                    ...
                    elif intent != "general_chat": # Ensure we have a response if generation failed but wasn't general chat
                         ai_text_response = "Couldn't generate the design visuals. Please try describing it differently."


                elif intent == "order_request":
                    # Check if artwork exists before proceeding
                    if not draft_details.get('artwork_image_url'):
                         ...
                         intent = "general_chat" # Stay in designing state
                    else:
                         next_state = "collecting_variant"
                         draft_details = {'artwork_image_url': draft_details.get('artwork_image_url')} # Reset draft, keep artwork
                         log.info(f"Session {session_id} state -> {next_state}")
                # Handle design_revision, general_chat - no state change needed here

            # --- ORDER DATA COLLECTION STATES (with validation) ---
            elif current_state == "collecting_variant":
                if intent == "collect_variant":
                    # Extract product, size, color (LLaMA should provide these)
                    ...
                    if prod_type and size and color:
                        ...
                        if variant_id:
                            ...
                            # Store product name from lookup? (Requires modifying _find_printful_variant_id or another call)
                            # draft_details['product_name'] = "Looked up product name" # Placeholder
                ...
                # else: implicit clarification request handled below
                ...
                if intent == "collect_quantity":
                    ...
                    try:
                        ...
                        if quantity > 0 and quantity <= 1000: # Printful item limit - assuming 1000 practical limit
                ...
                if intent == "collect_state_zip":
                    ...
                    country_code = draft_details.get('recipient_country_code') # Should exist from previous step
                    if not country_code: # Should not happen if flow is correct
                         ...
                         # Check if state validation failed where it was needed
                         ...
                         # Check if state is missing where needed (and user provided something, likely just ZIP)
                             ...
                             # Validation passed or state not strictly required/validated
                             draft_details['recipient_state_code'] = validated_state # Might be None if not required/found
            ...
            # --- Generic order modification/cancel/clarification ---
                 ...
                 # LLaMA asks 'what to change'. User responds. Next turn, LLaMA should ideally detect
                 # what they want to change and maybe suggest jumping state, or handle it via general_chat.
                 # For simplicity, we just let LLaMA handle the clarification for now.
                 pass # Keep current state, use LLaMA's response_text.
            elif current_state.startswith("collecting_") or current_state == "ready_for_checkout":
                 # If LLaMA didn't understand the input for the current collection step
                 ask_user_to_clarify = True # Trigger clarification logic below
            ...
            next_state = current_state # Revert state change on error

        # 6. Update Session State and Draft Details in DB
        if next_state != session.state or draft_details != session.draft_order_details:
             session.state = next_state
             session.draft_order_details = draft_details
             log.info(f"Updating session {session_id}: state='{next_state}'") # Avoid logging full draft details unless debugging
        ...
        try:
             await self.db.commit() # Commit session state changes and messages
        except Exception as db_err:
             log.error(f"Database commit error in handle_chat_message for session {session_id}: {db_err}", exc_info=True)
             await self.db.rollback() # Rollback changes
             # Raise HTTPException to inform client of failure
    ...
    # --- Checkout Initiation (with validation from draft) ---
    async def initiate_checkout(self, req: CheckoutRequest) -> CheckoutResponse:
        ...
        draft_details = session.draft_order_details if isinstance(session.draft_order_details, dict) else {}
        log.info(f"Initiating checkout for session {session_uuid} with draft: {draft_details}") # Log draft for debugging checkout

        # --- Validate and Construct OrderItem/Recipient ---
        try:
            ...
            # Construct product name using collected info
            ...
            # Validate recipient details more strictly before API calls
            recipient_data = {
                ...
                "state_code": draft_details.get('recipient_state_code'), # Can be None
                "zip": draft_details.get('recipient_zip') # Can be None
            }
            ...
            # Ensure state code is present if required by country
            ...
            recipient = Recipient(**recipient_data) # Use Pydantic for final validation

        except (ValidationError, ValueError, KeyError) as e:
            log.error(f"Checkout validation failed for session {session_uuid}: {e}. Draft: {draft_details}")
            # Optionally reset state to re-collect problematic data
            # session.state = "collecting_recipient_name" # Example
        ...
        # Upload artwork to Printful (may raise HTTPException)
        ...
        # Get costs from Printful (may raise HTTPException)
        costs = await _get_printful_costs(order_item, recipient, printful_headers)

        # --- Database and Stripe Transaction ---
        async with self.db.begin_nested(): # Use nested transaction for atomicity
            new_order = OrderRecord(user_id=self.user.id, product_name=order_item.product_name, image_id=artwork_msg.id)
            self.db.add(new_order); await self.db.flush() # Get new_order.id
            ...
            self.db.add(new_payment); await self.db.flush() # Get new_payment.id

            # Update PI metadata with internal payment ID (best effort)
            try:
                stripe.PaymentIntent.modify(intent.id, metadata={**intent.metadata, "order_payment_id": str(new_payment.id)})
            except stripe.error.StripeError as e:
                log.warning(f"Failed to modify Stripe PI metadata for {intent.id}: {e}") # Non-critical

            # Clear draft details and reset state after successful initiation
    ...
    # --- Stripe Webhook Handler (Error handling improved slightly) ---
    async def handle_stripe_webhook(self, payload: bytes, sig_header: str) -> dict:
        ...
        if event.type == "payment_intent.succeeded":
            ...
            printful_headers = {"Authorization": f"Bearer {PRINTFUL_API_KEY}"}
            asyncio.create_task(_submit_order_to_printful(payment, order, printful_headers, self.db)) # Background task
            ...
            if payment_id_str:
                ...
                if payment and payment.status == "pending_payment":
                    ...
                    payment.status = "failed"; payment.error_message = fail_msg[:255] # Limit error message length
...
# ===================================================================
# FASTAPI ROUTER (Wrappers - Mostly Unchanged)
# ===================================================================
...
async def start_new_chat_session(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        # Start in 'designing' state with empty draft
...
async def chat_with_ai(prompt: str = Form(...), session_id: uuid.UUID = Form(...), uploaded_image: Optional[UploadFile] = File(None), db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # The handler now contains significant logic and error handling
...
async def get_chat_history(session_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Convert ORM objects to dicts for JSON response (Pydantic can often handle this too)
...
async def initiate_checkout_endpoint(req: CheckoutRequest, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # Handler now contains validation and API calls
...
async def stripe_webhook_endpoint(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # Handler contains logic to update payment/order status and trigger fulfillment
    return await handler.handle_stripe_webhook(payload, sig_header)


@router.post("/ai/upload-image")
async def upload_user_image_endpoint(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Handles user image uploads (for reference/design)."""
    handler = GlitchApeCentralHandler(db=db, user=current_user)
...
async def get_image_endpoint(filename: str):
    ...
    # (Sanitization and MIME type logic unchanged)
    if ".." in filename or filename.startswith(("/", "\\")): raise HTTPException(400, "Invalid filename.")
    file_path = os.path.join(UPLOAD_DIR, filename)
    if not os.path.exists(file_path): raise HTTPException(404, "Image not found")
    ext = filename.split(".")[-1].lower(); media_type = MIME_TYPES.get(ext, "application/octet-stream")
