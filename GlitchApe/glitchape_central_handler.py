...
from google.api_core import exceptions as google_api_core_exceptions # Fallback for other core errors
...
# --- LLaMA 4 (Text Model) Configuration (REMOVED) ---
# ... (All LLaMA config removed) ...
...
gemini_client: Optional[genai.Client] = None # <-- MODIFIED: Now genai.Client
if GEMINI_API_KEY:
    try:
        # --- AUTHENTICATION FIX (REFACTORED) ---
        # The new genai.Client() automatically picks up
        # the GEMINI_API_KEY environment variable.
        # genai.configure() is no longer needed.
...
# --- Gemini System Prompt (RENAMED from LLaMA) ---
...
# --- MODIFIED: _analyze_image_with_vision_model (REFACTORED for google-genai) ---
async def _analyze_image_with_vision_model(image_bytes: bytes, mime_type: str, user_prompt: str) -> str:
    """Uses Gemini SDK to describe image content from bytes."""
    global gemini_client
    if not gemini_client: return "ERROR: Gemini Client not initialized."

    log.info(f"Calling Gemini ({PRIMARY_LLM_MODEL}) via SDK for analysis on image bytes ({len(image_bytes)})...")
    ...
    try:
        ...
        # REFACTORED: Use client.models.generate_content
        def sync_generate():
            return gemini_client.models.generate_content(
                model=PRIMARY_LLM_MODEL,
                contents=[image_part, vision_analysis_prompt],
                config=genai_types.GenerateContentConfig(max_output_tokens=1024, temperature=0.3), # <-- MODIFIED
                request_options={'timeout': GEMINI_TIMEOUT}
            )
        response = await asyncio.to_thread(sync_generate)

        analysis_text = response.text
        if not analysis_text: log.warning(f"Gemini returned empty analysis."); return "Vision model returned empty analysis."
        log.info(f"Gemini analysis successful.")
...
async def _get_printful_costs(item: OrderItem, recipient: Recipient, headers: Dict[str, str]) -> Dict[str, Any]:
    """Calls Printful API to get costs (item, shipping, tax)."""
    # (Implementation unchanged)
    payload = {
        "recipient": recipient.model_dump(exclude_none=True),
        "items": [item.model_dump()],
        "currency": "USD" # Assume USD for Printful API calls
    }
    
    try:
        async with httpx.AsyncClient(timeout=PRINTFUL_TIMEOUT) as client:
            # We use /orders/estimate to calculate costs
            resp = await client.post(f"{PRINTFUL_API_URL}/orders/estimate", headers=headers, json=payload)
            resp.raise_for_status()
        
        data = resp.json().get("result")
        if not data or not isinstance(data, list) or not data[0].get("cost"):
            log.error(f"Printful estimate missing cost data. Resp: {resp.text}")
            raise ValueError("Missing cost details from Printful API.")

        # Printful returns a list of cost estimates, we take the first/only one
        cost_details = data[0]
        
        # Calculate total cost in cents to avoid float issues later
        item_cost = cost_details.get("item_cost", 0.0)
        shipping_cost = cost_details.get("shipping_cost", 0.0)
        # tax_cost might be null/missing if no tax is applied
        tax_cost = cost_details.get("tax_cost", 0.0) or 0.0 
        ...
        log.error(f"Printful cost calculation HTTP error: {e.response.status_code} - {e.response.text[:200]}")
        # Pass a specific Printful error message if available
        try:
            error_data = e.response.json().get("result", {})
            error_message = error_data.get("error") or error_data.get("message")
        except:
            error_message = f"Printful API Error ({e.response.status_code})"
        raise HTTPException(status_code=502, detail=error_message)
    except httpx.RequestError as e:
        log.error(f"Network error getting Printful costs: {e}")
        raise HTTPException(status_code=504, detail="Network error contacting Printful.")
    except Exception as e:
        log.error(f"Unexpected error in Printful cost calculation: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to calculate Printful costs: {str(e)[:100]}")
...
async def _submit_order_to_printful(payment: OrderPayment, order: OrderRecord, headers: Dict[str, str], db: AsyncSession):
    ...
    # (Implementation largely unchanged, relies on updated helpers)
    try:
        # 1. Prepare data
        recipient = json.loads(payment.recipient_json)
        file_id_map = json.loads(payment.printful_file_ids_json)
        
        # In this implementation, we assume only one file (the main artwork)
        artwork_printful_id = list(file_id_map.values())[0]
        
        # The file placement for Printful's order submission
        # NOTE: This assumes the design is always on the 'front'
        # A more complex system would read the actual placement_id from memory
        # but for this version, we stick to the simplest one-file, one-placement.
        printful_files = [{"id": artwork_printful_id, "type": "front"}] 
        
        # Create the order item payload
        order_item = {
            "variant_id": payment.variant_id,
            "quantity": payment.quantity, # Use quantity from payment record
            "files": printful_files, 
            "name": order.product_name,
            "external_id": f"GLA-ITEM-{order.id}"
        }
        
        payload = {
            "recipient": recipient, 
            "items": [order_item], 
            "external_id": f"GLA-{order.id}",
            "packing_slip": { # Add a simple packing slip for branding
                "email": "support@glitchape.fun",
                "phone": "+1-800-GLITCH",
                "message": "Thanks for glitching out with us! Enjoy your custom gear. 🚀"
            }
        }
        
        # 2. Submit as Draft Order
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if not printful_order_id:
                raise ValueError("Printful did not return order ID after creation.")
            
            log.info(f"Created Printful draft order {printful_order_id} for internal order {order.id}")
            
            # 3. Confirm (Confirming also charges the saved payment method on Printful if any, but we are webhook-driven)
            ...
            if not confirmed_order or not confirmed_order.get("id"):
                raise ValueError("Printful did not return order details after confirmation.")
        ...
        payment.status = "printful_error"
        if isinstance(e, httpx.HTTPStatusError):
            try:
                # Try to get specific error from Printful response
                printful_err = e.response.json().get("result", {}).get("message")
                payment.error_message = printful_err or err_msg
            except:
        ...
        # Re-raise as an HTTP error for the API consumer
        raise HTTPException(status_code=502, detail=f"Order submission to Printful failed: {payment.error_message}")
...
class GlitchApeCentralHandler:
    def __init__(self, db: AsyncSession, user: Optional[User] = None):
        self.db = db
        self.user = user
        # Immediately start population of Printful location data in the background
        # It's okay if it hasn't finished when the first chat message arrives,
        # as the validation functions gracefully fall back or check the cache if it's empty.
    ...
    async def _get_memory_context(self, session: ChatSession) -> Tuple[Dict[str, Any], str]:
        ...
        if not draft_details.get('size_input') or not draft_details.get('color_input'):
            ...
            # This should only happen if validation failed to find a match
        ...
        # Conditional State/Zip check
        elif draft_details.get('recipient_country_code') in COUNTRIES_NEEDING_STATES and not draft_details.get('recipient_state_code'):
            missing_fields.append("recipient_state_code")
        elif not draft_details.get('recipient_zip'):
            missing_fields.append("recipient_zip")
    ...
    async def _check_and_set_checkout_ready(self, session: ChatSession, draft_details: Dict[str, Any]) -> bool:
        """Checks if all CHECKOUT_REQUIRED_FIELDS are present and valid."""
        is_ready = True
        
        # 1. Check for presence of required fields
        for field in CHECKOUT_REQUIRED_FIELDS:
            if field not in draft_details:
                is_ready = False
                break
        
        # 2. Check conditional state field
        country_code = draft_details.get('recipient_country_code', '').upper()
        if country_code in COUNTRIES_NEEDING_STATES and not draft_details.get('recipient_state_code'):
            is_ready = False
        
        # 3. Check for positive integer quantity
        try:
            quantity = int(draft_details.get('quantity', 0))
            if quantity <= 0: is_ready = False
        except (ValueError, TypeError):
            is_ready = False
        
        # 4. Set state if ready, unset otherwise
        if is_ready:
            if session.state != "ready_for_checkout":
                session.state = "ready_for_checkout"
                log.info(f"Session {session.id} state set to 'ready_for_checkout'.")
            return True
        else:
            # If we were ready but now aren't (due to a revision), revert state
            if session.state == "ready_for_checkout":
                session.state = "designing"
                log.info(f"Session {session.id} state reverted to 'designing'.")
            return False

    async def _check_and_trigger_mockup(self, draft_details: Dict[str, Any], force_regenerate: bool = False) -> Optional[str]:
        """
        If all necessary data for mockup is present in memory.
        If yes, generates mockup, saves URL to draft_details, and returns URL.
        Returns None if not ready or on non-HTTP error. Raises HTTPException on API/HTTP failures.
        """
        try:
            ...
            # 2. Check if we've already generated this exact configuration
            # To avoid excessive API calls, we use a simple config key
            current_config = f"{product_input}-{placement_id}-{width_in_str}-{offset_y_in_str}-{design_url}"
            if 'last_mockup_config' in draft_details and draft_details['last_mockup_config'] == current_config and not force_regenerate:
                existing_mockup_url = draft_details.get('mockup_image_url')
                if existing_mockup_url:
                    log.info("Mockup config identical, skipping regeneration.")
                    return existing_mockup_url

            # 3. Validate data
            product_id = PRODUCT_TYPE_TO_PRINTFUL_ID.get(product_input.lower().strip())
            if not product_id:
                # This should have been caught by the LLM/variant validation earlier
            ...
            if not (0.1 < width_in <= 20.0 and 0.0 <= offset_y_in <= 20.0):
                raise ValueError("Invalid dimensions (must be 0.1-20 inches wide and 0-20 inches down).")
            ...
            # 5. Save URL and config to memory and return
            draft_details['mockup_image_url'] = mockup_url
            draft_details['last_mockup_config'] = current_config
            ...
            # This is a user-facing error because the data they provided is bad
            # Clear the mockup to force regeneration once data is fixed
            draft_details.pop('mockup_image_url', None) 
            draft_details.pop('last_mockup_config', None) 
            # Return None to suppress the generic system error response and let the LLM handle it
            return None 
        except HTTPException:
            ...
            draft_details.pop('mockup_image_url', None)
            draft_details.pop('last_mockup_config', None)
            return None # Return None on unexpected system error

    # --- REPLACED: _prepare_llama_history with _prepare_gemini_history ---
    async def _prepare_gemini_history(self, session: ChatSession, draft_details: Dict[str, Any], missing_info_str: str) -> List[Dict[str, Any]]:
        """Prepares the conversation history and memory for the Gemini API call."""
        
        # 1. Fetch recent chat messages (limit 10 for context)
        q = select(ChatMessage).where(ChatMessage.session_id == session.id).order_by(ChatMessage.created_at.asc()).limit(10)
        result = await self.db.execute(q)
        messages = result.scalars().all()

        # 2. Build the initial system prompt context
        memory_json = json.dumps(draft_details, indent=2, default=str)
        context = f"[MEMORY: {memory_json}]\n\n[MISSING_INFO: {missing_info_str}]"
        
        # 3. Format history for Gemini SDK
        # REFACTORED: The new SDK guidelines recommend "system" instructions
        # go into the GenerateContentConfig, not the history.
        # However, this "memory" block is dynamic per-call, so
        # we will prepend it to the user's *first* message.
        
        gemini_history: List[Dict[str, Any]] = []
        
        for i, msg in enumerate(messages):
            api_role = "model" if msg.role == "ai" else "user" # Use 'model' for AI role
            
            content = msg.content
            
            # Condense the long image analysis for history efficiency
            if content.startswith("[IMAGE ANALYSIS:") and len(content) > 300:
                 # Extract user request if present
                user_req = content.split("User request:", 1)[-1].strip()
                content = f"[Image analysis provided] User request: {user_req}"

            # Prepend the dynamic context to the first user message
            # if i == 0 and api_role == "user":
            #     content = f"{context}\n\n{content}"
            # ---
            # RE-EVALUATION: The guidelines show 'system_instruction' for prompts,
            # but this is *dynamic context*. The old method of a 'system' role
            # message is not supported. Prepending to the *current* user prompt
            # is better. This function prepares *history*, so the old
            # 'system' role message is actually NOT supported.
            
            # Let's adjust. The history should just be user/model.
            # The *dynamic* context will be added in _call_gemini_brain.
            # No, _prepare_gemini_history *is* adding the context.
            
            # Sticking to the original logic, but changing "system" role to "user"
            # as the new SDK doesn't have a "system" role in the history.
            # The 'system_instruction' is for the *main* prompt.
            # This is a tricky migration.
            
            # ---
            # NEW STRATEGY: The original `_prepare_gemini_history` put MEMORY
            # in a "system" role. The new SDK doesn't use that.
            # We will pass the MEMORY context as part of the `system_instruction`
            # in `_call_gemini_brain` *along with* the main system prompt.
            
            # ---
            # RE-RE-EVALUATION: The *guidelines* show `system_instruction` for
            # static prompts. The `contents` list is for user/model turns.
            # The `GlitchApeCentralHandler`'s *old* logic
            # (line 1121) put the context in a "system" message.
            # This is no longer valid.
            
            # The *correct* way to handle dynamic context is to put it
            # in the `system_instruction` parameter of `GenerateContentConfig`.
            
            # ---
            # FINAL STRATEGY:
            # 1. `_prepare_gemini_history` will NO LONGER add the context.
            #    It will *only* return the user/model message history.
            # 2. `_call_gemini_brain` will construct the *full* system prompt
            #    (static prompt + dynamic context) and pass it to
            #    `system_instruction`.

            # --- Refactoring _prepare_gemini_history ---
            gemini_history = []
            for msg in messages:
                ...
                gemini_history.append({"role": api_role, "parts": [{"text": content}]})

        # The context (memory, missing_info) is now handled in _call_gemini_brain
        return gemini_history

    # --- REPLACED: _call_llama_brain with _call_gemini_brain (REFACTORED) ---
    async def _call_gemini_brain(
        self, 
        prompt_parts: List[str | genai_types.Part], 
        history: List[Dict[str, Any]],
        draft_details: Dict[str, Any], # <-- ADDED
        missing_info_str: str # <-- ADDED
    ) -> Dict[str, Any]:
        """Calls Gemini via SDK, handles JSON extraction and errors."""
        global gemini_client
        if not gemini_client:
            raise HTTPException(503, "Gemini Client not initialized. Check API Key.")

        # 1. Combine history with the current prompt
        sdk_messages = history + [{"role": "user", "parts": prompt_parts}]

        # 2. Construct dynamic system prompt
        memory_json = json.dumps(draft_details, indent=2, default=str)
        dynamic_context = f"[MEMORY: {memory_json}]\n\n[MISSING_INFO: {missing_info_str}]"
        full_system_prompt = f"{GEMINI_SYSTEM_PROMPT}\n\n{dynamic_context}"

        # 3. Configure for JSON output and inject the system prompt
        config = genai_types.GenerateContentConfig( # <-- MODIFIED
            response_mime_type="application/json", # Force JSON output
            temperature=0.2, # Lower temp for more predictable JSON/intent
            system_instruction=full_system_prompt # <-- MODIFIED
        )
        ...
        try:
            ...
            # Use regex to robustly extract a JSON object
            json_match = re.search(r'\{.*\}', gemini_response_content, re.DOTALL)
            
            if not json_match:
                log.error(f"Gemini failed to include a JSON object in: {gemini_response_content[:500]}")
                # Fallback to a basic chat response if JSON is mandatory for the flow
                return {"intent": "general_chat", "response_text": f"// Marvick's core logic unit glitched. I received an incomplete response. Let's try that again. What's next?"}

            action_json = json.loads(json_match.group(0))
            return action_json

        except Google_ResourceExhaustedError: # <-- MODIFIED
            ...
            raise HTTPException(502, f"AI Service API Error: {e}")
        except json.JSONDecodeError as e:
            log.error(f"AI returned invalid JSON: {e}. Content: {gemini_response_content[:500]}")
            # This is a critical failure. Attempt a fallback response.
            return {"intent": "general_chat", "response_text": "// ERROR: My internal data-stream corrupted. Let's reset the command. What were you saying?"}
        except Exception as e:
            log.error(f"Unexpected error in _call_gemini_brain: {e}", exc_info=True)
            raise HTTPException(500, "Internal error calling AI service.")
    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        """Central orchestrator for handling a user's chat message."""
        if not self.user: raise HTTPException(401, "User not authenticated")

        # 1. Fetch Session and Messages
        session = await self.db.get(ChatSession, session_id)
        if not session or session.user_id != self.user.id: raise HTTPException(404, "Session not found.")
        
        # 2. Add user message to history *before* processing
        user_msg = ChatMessage(session_id=session_id, user_id=self.user.id, role="user", content=prompt)
        self.db.add(user_msg)

        # 3. Handle Uploaded Image (This logic is complex due to Cloudinary + Vision Analysis)
        draft_details = session.draft_order_details if isinstance(session.draft_order_details, dict) else {}
        gemini_prompt_parts: List[str | genai_types.Part] = [prompt]
        image_previews: List[ImagePreview] = []
        
        image_bytes_for_analysis: Optional[bytes] = None
        mime_type_for_analysis: Optional[str] = None
        ...
        if uploaded_image and uploaded_image.filename:
            try:
                ...
                log.info(f"User {self.user.id} uploaded to {db_image_url}. Fetching bytes for analysis...")

                # --- THIS IS A CRITICAL MEMORY UPDATE ---
                draft_details['artwork_image_url'] = db_image_url
                draft_details.pop('mockup_image_url', None)
                
                # --- NEW: Fetch bytes ONCE from Cloudinary for Vision Model ---
                async with httpx.AsyncClient() as client:
                    img_resp = await client.get(db_image_url, timeout=30, follow_redirects=True)
                    img_resp.raise_for_status()
                    image_bytes_for_analysis = img_resp.content
                    mime_type_for_analysis = img_resp.headers.get('content-type', uploaded_image.content_type)
                
                # Create the image part for the main prompt
                # THIS IS THE LINE THAT CAUSED THE ORIGINAL CRASH
                # It will now work because 'genai_types' is imported from 'google.genai.types'
                image_part = genai_types.Part.from_bytes(
                    data=image_bytes_for_analysis, 
                    mime_type=mime_type_for_analysis
                )
                gemini_prompt_parts = [image_part, prompt] # Image goes before text

                # --- Run Vision Analysis (passing bytes) ---
                image_analysis_text = await _analyze_image_with_vision_model(
                    image_bytes_for_analysis, 
                    mime_type_for_analysis, 
                    prompt
                ) # <-- MODIFIED CALL

                if image_analysis_text.startswith("ERROR:"):
                    analysis_error_message = image_analysis_text
                    image_analysis_text = None
                else:
                    log.info("Vision analysis successful.")
                    draft_details['analysis_text'] = image_analysis_text
                    # Inject analysis into the chat history for LLM context
                    user_msg.content = f"[IMAGE ANALYSIS: {image_analysis_text}] User request: {prompt}"

            except HTTPException as e:
                raise e # Propagate HTTP errors from validation/upload
            except Exception as e:
                log.error(f"Upload/Vision error: {e}", exc_info=True)
                analysis_error_message = "Image processing failed. Try uploading again."
                # Don't fail the whole request, but clear the artwork URL so the user has to try again
                draft_details.pop('artwork_image_url', None)

        # 4. Prepare Memory Context and History
        (draft_details, missing_info_str) = await self._get_memory_context(session)
        # REFACTORED: History no longer contains context
        history = await _prepare_gemini_history(session, draft_details, missing_info_str) 
        
        # 5. Call LLM Brain (REFACTORED: Pass context here)
        action_json = await _call_gemini_brain(
            gemini_prompt_parts, 
            history,
            draft_details,
            missing_info_str
        ...
        # 6. Intent Handling and Orchestration
        ...
        try:
            if intent == "general_chat":
                # General chat, no state or memory change
                pass 
                ...
                if not design_prompt:
                    ...
                    # Clear old mockup as artwork changed
                    draft_details.pop('mockup_image_url', None) 
                    draft_details.pop('last_mockup_config', None)
            ...
            elif intent == "image_analysis_confirm":
                # User uploaded an image, analysis ran, and LLM is confirming.
                # The artwork_image_url was already set in Step 3.
                # We just add the preview, unless an error occurred during analysis
                if analysis_error_message:
                    ai_text_response = analysis_error_message
                    draft_details.pop('artwork_image_url', None)
                elif 'artwork_image_url' in draft_details:
                    image_previews.append(ImagePreview(view_name="Uploaded Artwork", url=draft_details['artwork_image_url']))
                else:
                    log.warning(f"Session {session_id}: LLM used 'image_analysis_confirm' but no artwork_image_url in memory.")
                    ai_text_response = "Something went wrong, I lost the image. Can you upload it again?"
                ...
                # --- Specific Validation Logic (Order Matters) ---
                
                # A. Validate Quantity
                if 'quantity' in updated_memory:
                    try:
                        quantity = int(updated_memory['quantity'])
                        if quantity <= 0:
                            raise ValueError("Quantity must be a positive number.")
                    except (ValueError, TypeError) as e:
                        validation_clarification = f"Error: Quantity input '{updated_memory['quantity']}' is invalid. Please provide a whole number greater than zero."
                        updated_memory.pop('quantity', None)

                # B. Address Validation (Country/State)
                if 'recipient_country_code' in updated_memory:
                    country_code = _validate_country_code(updated_memory['recipient_country_code'])
                    if not country_code:
                        validation_clarification = f"Error: I don't recognize country '{updated_memory['recipient_country_code']}'. Please use the 2-letter ISO code (e.g., US, CA) or the full name."
                        updated_memory.pop('recipient_country_code', None)
                    else:
                         updated_memory['recipient_country_code'] = country_code # Normalize
                         # If country changes, clear state/zip until they provide it again
                         if draft_details.get('recipient_country_code') != country_code:
                              updated_memory.pop('recipient_state_code', None)
                              updated_memory.pop('recipient_zip', None)

                if 'recipient_state_code' in updated_memory:
                    current_country = updated_memory.get('recipient_country_code', draft_details.get('recipient_country_code'))
                    state_code = updated_memory['recipient_state_code']
                    
                    if current_country in COUNTRIES_NEEDING_STATES:
                        validated_state = _validate_state_code(current_country, state_code)
                        if not validated_state:
                             validation_clarification = f"Error: State/Province '{state_code}' is invalid for {current_country}. Please use the standard code."
                             updated_memory.pop('recipient_state_code', None)
                        else:
                             updated_memory['recipient_state_code'] = validated_state # Normalize

                # C. Update the memory
                draft_details.update(updated_memory)
                
                # D. Re-validate Variant ID if product/size/color changed
                if variant_keys_updated:
                    (success, msg) = await _validate_and_update_variant(draft_details)
                    if not success:
                        validation_clarification = msg # Overrides AI's response with validation error
                        # Clear mockup as product/variant is now invalid
                        draft_details.pop('mockup_image_url', None)
                        draft_details.pop('last_mockup_config', None)
                
                # E. Trigger Mockup if placement data changed AND variant is valid
                mockup_url = None
                if placement_keys_updated and draft_details.get('variant_id'):
                    try:
                        mockup_url = await self._check_and_trigger_mockup(draft_details, force_regenerate=True)
                    except HTTPException as e:
                        validation_clarification = f"// Mockup Generator failed: {e.detail}"
                        mockup_url = None
                        draft_details.pop('mockup_image_url', None)

                if mockup_url:
                    image_previews.append(ImagePreview(view_name="Mockup Preview", url=mockup_url))
                # Add existing mockup URL if no regeneration happened but one exists
                elif draft_details.get('mockup_image_url') and not validation_clarification:
                     image_previews.append(ImagePreview(view_name="Mockup Preview", url=draft_details['mockup_image_url']))
            
            elif intent == "order_cancel":
                # Clear all draft details
                session.draft_order_details = {}
                session.state = "designing"
                log.info(f"Session {session_id} cancelled. State reset.")
            
            else:
                log.warning(f"Session {session_id}: Unhandled intent: {intent}")
                # Fallback to chat
                ai_text_response = f"// Glitch: I didn't recognize that command ({intent}). Try phrasing it differently?"
                intent = "general_chat"
                
        except HTTPException:
            raise # Propagate API errors
        except Exception as e:
            log.error(f"Error during intent processing for session {session_id}: {e}", exc_info=True)
            # Rollback any in-memory changes to be safe before database commit
            await self.db.rollback() 
            raise HTTPException(status_code=500, detail="Internal error during AI orchestration.")
        ...
        try:
            is_ready = await self._check_and_set_checkout_ready(session, draft_details)
            if is_ready:
                # Only append if the AI isn't already saying it
                if "checkout" not in ai_text_response.lower() and "complete" not in ai_text_response.lower():
                    ai_text_response += " // All data compiled! ✨ Hit **'Checkout'** to bridge to payment!"
            # If the session state was "ready_for_checkout" but the check failed, 
            # the helper function has already reverted it to "designing"
    ...
    # --- Checkout Initiation (Logic largely unchanged, but helpers are different) ---
    async def initiate_checkout(self, req: CheckoutRequest) -> CheckoutResponse:
        ...
        # 1. Final Data Validation & Structuring
        draft_details = session.draft_order_details if isinstance(session.draft_order_details, dict) else {}
        if session.state != "ready_for_checkout":
             # Force a final check to ensure all data is compiled
             await self._check_and_set_checkout_ready(session, draft_details)
             if session.state != "ready_for_checkout":
                  raise HTTPException(status_code=400, detail="Order not ready for checkout. Missing information. Please check chat history.")
        ...
        try:
            ...
            recipient_data = {
                ...
                "address2": draft_details.get('recipient_address2'), # Optional
                "city": draft_details.get('recipient_city'),
                "country_code": draft_details.get('recipient_country_code'),
                # State code is conditional, retrieve if present
                "state_code": draft_details.get('recipient_state_code'), 
                "zip": draft_details.get('recipient_zip'),
                "email": self.user.email, # Use user's registered email
                "phone": None # Optional
            }
            
            # Check mandatory recipient fields
            ...
            order_item = OrderItem(
                ...
                product_name=draft_details.get('product_input'),
                quantity=int(draft_details.get('quantity', 0))
            )
        ...
        # 3. Upload Artwork to Printful (if not done yet, or if URL is new)
        ...
        # Find the ChatMessage that holds the artwork URL
        ...
        # 4. Create OrderRecord (Internal)
        # Use a database transaction for atomic order creation and payment setup
        async with self.db.begin_nested():
            ...
            await self.db.flush() # Get the new_order.id

            # 5. Create OrderPayment Record
            recipient_json = json.dumps(recipient.model_dump(exclude_none=True))
            new_payment = OrderPayment(
                order_id=new_order.id,
                variant_id=order_item.variant_id,
                quantity=order_item.quantity,
                amount=costs["total_cost"],
                amount_cents=costs["total_cents"],
                currency=costs["currency"],
            ...
            await self.db.flush() # Get the new_payment.id

            # 6. Create Stripe Payment Intent
            intent = stripe.PaymentIntent.create(
                amount=costs["total_cents"],
                currency=costs["currency"],
                # IMPORTANT: Associate payment intent with our internal payment record ID
                metadata={
                    "order_id": str(new_order.id),
                    "user_id": str(self.user.id),
                    "order_payment_id": str(new_payment.id)
                },
                # Use automatic payment methods for simplicity
                automatic_payment_methods={"enabled": True},
            )

            # Update Payment record with Stripe Intent details
            new_payment.stripe_payment_intent_id = intent.id
            session.state = "awaiting_payment" # Update session state
            
            log.info(f"Created Stripe Payment Intent {intent.id} for Order {new_order.id}. Session state: awaiting_payment.")
    ...
    # --- Stripe Webhook Handler (Logic largely unchanged) ---
    async def handle_stripe_webhook(self, payload: bytes, sig_header: str) -> dict:
        ...
        try:
            ...
            log.warning("Stripe hook invalid payload."); raise HTTPException(400, "Invalid payload")
        except stripe.error.SignatureVerificationError:
            log.warning("Stripe hook invalid signature."); raise HTTPException(400, "Invalid signature")
        except Exception as e:
            log.error(f"Error constructing Stripe event: {e}"); raise HTTPException(500, "Webhook processing error")
        ...
        if event.type == "payment_intent.succeeded":
            if not payment_id_str: 
                log.error(f"Stripe event {event.id} (succeeded) missing 'order_payment_id'."); return {"status": "error", "reason": "Missing metadata"}
            ...
            if payment.status != "pending_payment" and payment.status != "awaiting_payment": 
                log.info(f"Stripe event {event.id} already processed (status: {payment.status})."); return {"status": "ok", "message": "Already processed"}
            ...
            if not order: 
                ...
                log.error(f"Stripe event {event.id} succeeded but OrderRecord {payment.order_id} not found."); return {"status": "error", "reason": "Order record not found"}
            
            order.status = "payment_succeeded" # Update Order status
            
            # --- FULFILLMENT: Submit to Printful ---
            try:
                printful_headers = {"Authorization": f"Bearer {PRINTFUL_API_KEY}"}
                # This helper now updates order/payment records internally
                await _submit_order_to_printful(payment, order, printful_headers, self.db)
                
            except HTTPException as e:
                # If Printful submission fails, the payment record is set to printful_error inside the helper
                log.error(f"Critical Printful submission error after Stripe success: {e.detail}")
                # The user gets a generic success response from the webhook, 
                # but internal status is 'printful_error' for manual review.
                pass 
            except Exception as e:
                 # Catch-all for fulfillment
                log.critical(f"Unhandled fulfillment error: {e}", exc_info=True)
                payment.status = "error"
                payment.error_message = f"Unhandled fulfillment error: {str(e)[:100]}"
                await self.db.commit()
                pass
            
            await self.db.commit()
            return {"status": "ok", "message": "Payment recorded and fulfillment triggered."}

        # 3. Process Failure Event
        elif event.type in ["payment_intent.payment_failed", "payment_intent.canceled"]:
            if not payment_id_str: return {"status": "ok", "message": "Ignored (no metadata)"}
            try: payment_id = uuid.UUID(payment_id_str)
            except ValueError: return {"status": "error", "reason": "Invalid payment_id format"}
            
            payment = await self.db.get(OrderPayment, payment_id)
            if payment:
                payment.status = "failed" if event.type == "payment_intent.payment_failed" else "cancelled"
                payment.error_message = intent.last_payment_error.message if intent.last_payment_error else f"Payment {event.type.split('.')[-1]}."
                
                order = await self.db.get(OrderRecord, payment.order_id)
                if order: order.status = payment.status # Update Order status
                
                await self.db.commit()
                log.info(f"Payment Intent {intent.id} failed/cancelled. Order {payment.order_id} updated.")
            return {"status": "ok", "message": "Payment record updated."}
        
        # 4. Ignore other events
        else:
            log.info(f"Ignored Stripe event type: {event.type}")
...
async def start_new_chat_session(
    ...
    # (Implementation unchanged)
    try:
        new_session = ChatSession(user_id=current_user.id, title=f"Chat {datetime.now():%Y-%m-%d %H:%M}", state="designing")
        db.add(new_session)
        
        initial_msg = ChatMessage(session_id=new_session.id, user_id=current_user.id, role="ai", content="// Welcome to GlitchApe! I'm Marvick. Let's design some cyberpunk threads. What kind of artwork should we start with? [Request, or Upload an Image]")
        db.add(initial_msg)
        
        await db.commit()
        log.info(f"New session started for user {current_user.id}: {new_session.id}")
        return {"session_id": str(new_session.id), "initial_message": initial_msg.content}
    except Exception as e:
        log.error(f"Error starting new session: {e}", exc_info=True)
        await db.rollback()
...
async def send_chat_message_endpoint(
    ...
    """Sends a message to an active chat session (and includes an optional file upload)."""
    handler = GlitchApeCentralHandler(db=db, user=current_user)
    # The handler now manages all the internal logic (AI call, file upload, state update)
    return await handler.handle_chat_message(session_id, prompt, uploaded_image)


@router.post("/checkout/initiate")
async def initiate_checkout_endpoint(
    req: CheckoutRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
) -> CheckoutResponse:
    """Initiates the checkout process by validating data, getting costs, and creating a Stripe Payment Intent."""
    handler = GlitchApeCentralHandler(db=db, user=current_user)
...
async def stripe_webhook_endpoint(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # (Implementation unchanged)
    # Note: We do not pass a 'user' to the handler for webhooks as it's a system call
