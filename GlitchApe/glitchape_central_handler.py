...
LLAMA_MODEL = "meta-llama/llama-4-marvick:free" # Using LLaMA 4 Marvick as requested
...
class GlitchApeCentralHandler:
    ...
    async def _call_llama_brain(
        ...
        try:
            ...
            if not llama_response_content:
                log.error(f"LLaMA response was empty. Data: {data}")
            ...
            log.error(f"LLaMA returned invalid JSON: {llama_response_content}")
            raise HTTPException(status_code=502, detail=f"LLaMA returned invalid JSON.")
        except httpx.HTTPStatusError as e:
            log.error(f"LLaMA API Error: {e.response.status_code} - {e.response.text}")
            raise HTTPException(status_code=e.response.status_code, detail=f"LLaMA API Error")
        except Exception as e:
    ...
    async def handle_chat_message(
        ...
        prompt: str,
        uploaded_image: Optional[UploadFile] = None # <-- CHANGED
    ) -> LiveChatResponse: # <-- CHANGED
        ...
        if uploaded_image and uploaded_image.filename:
            try:
                ...
                # Pass file-saving errors (e.g., 413 Too Large) back to the user
        ...
        try:
            ...
            return LiveChatResponse( # <-- CHANGED
                session_id=str(session_id),
                response_text=ai_text_response,
                image_url=None # <-- ADDED
            )
        ...
        try:
            ...
            ai_text_response = f"I tried to process your request, but an error occurred."
            if isinstance(e, HTTPException):
                ai_text_response = f"I tried, but hit a snag: {e.detail}"

        # --- ADDED: Extract the primary image URL for the response ---
        main_image_url = image_previews[0].url if image_previews else None

        # 6. Save the main AI text response
        ai_text_msg = ChatMessage( # <-- MODIFIED
            session_id=session_id, user_id=self.user.id, role="ai", content=ai_text_response,
            image_url=main_image_url # <-- ADDED
        )
        self.db.add(ai_text_msg)
        await self.db.commit()

        # 7. Return the consolidated response
        return LiveChatResponse( # <-- MODIFIED
            session_id=str(session_id),
            response_text=ai_text_response,
...
async def chat_with_ai(
    ...
    # ***REMOVED***: The try/except block for uuid.UUID(session_id) is no longer needed.
    # 'session_id' is now already a validated uuid.UUID object from the signature.
