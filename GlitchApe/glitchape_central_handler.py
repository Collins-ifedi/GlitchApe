...
MODIFIED (No-Vision Flow): User-uploaded images are no longer sent to a
          vision model for analysis. The AI is now prompted to
          immediately ask for confirmation of use and initial
          placement details.
"""
...
# --- LLaMA System Prompt (DYNAMIC AI FLOW UPGRADE) ---
LLAMA_SYSTEM_PROMPT = """
...
-   **Image Uploads:** If the user prompt contains `[IMAGE UPLOADED: ...]`, this is your **highest priority**. Your *only* action must be `{"intent": "general_chat", "response_text": "Got the image! Do you want to use this as the artwork for your design? If so, what **product** (t-shirt, hoodie) and **placement** (front, back) are you thinking?"}`. This single question confirms usage and gets the first pieces of info. The user's *next* reply will then be handled by `collect_order_details`.
-   **No False Uploads:** If the user *mentions* uploading (e.g., "uploaded", "I sent it") but the prompt does **NOT** contain the `[IMAGE UPLOADED: ...]` token, do **NOT** confirm receipt. Instead, use `general_chat` to guide them to use the upload button. Example: `{"intent": "general_chat", "response_text": "I don't see an image attached. Please use the upload button to send me your file."}`
...
# --- UNUSED: _analyze_image_with_vision_model ---
# This function is no longer called by handle_chat_message
# as per the new "no-analysis" flow.
...
class GlitchApeCentralHandler:
    ...
    async def _get_chat_history_for_context(self, session: ChatSession, memory_json: str, missing_info: str, limit: int = 10) -> List[Dict[str, str]]:
        ...
        llama_history = [{"role": "system", "content": "\n".join(context_lines)}]
        # --- END MODIFICATION ---
        ...
        for msg in reversed(messages):
            ...
            # --- MODIFIED: Removed image analysis context trimming ---
    ...
    async def _call_llama_brain(self, prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        # --- MODIFIED: Use dynamic Referer and standard headers (Addresses 403 Error) ---
        headers = { 
        ...
        try:
            ...
            # --- ######################################################## ---
            # Use a multi-step regex approach to find the *correct* JSON block,
            # ignoring any context echoed by the LLM (like [MEMORY: ...]).
            ...
            if json_match_markdown:
                ...
                # The LLM is instructed to put the JSON object LAST.
                # This regex finds all distinct {...} blocks.
                all_json_blocks = re.findall(r'(\{.*?\})', llama_response_content, re.DOTALL)
                
                if all_json_blocks:
                    # The last one is assumed to be the action JSON
                    json_str = all_json_blocks[-1]
            
            # 3. If no JSON string was found by either method, log and return a fallback.
            if not json_str:
                log.error(f"LLaMA failed to include a JSON object. Raw response: {llama_response_content}")
                return {
                    "intent": "general_chat",
            ...
            try:
                ...
                log.error(f"LLaMA invalid JSON detected. Err: {e}. Str: {json_str}. Full Raw: {llama_response_content}")
            ...
            # Log 403 Forbidden errors with more context
    ...
    # --- MAIN CHAT HANDLER (DYNAMIC AI FLOW UPGRADE) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        """Orchestrates chat, design, and validated order data collection."""
        ...
        # 2. Handle Upload (NO-VISION FLOW)
        if uploaded_image and uploaded_image.filename:
            try:
                ...
                log.info(f"User {self.user.id} uploaded to {db_image_url}.")
                ...
                # --- NEW/MODIFIED: Add an AI-like confirmation message to the chat history (This is the fix) ---
                # This message ensures the user *sees* the upload has been processed and is the new artwork.
                ai_upload_msg = ChatMessage(
                    session_id=session_id, user_id=self.user.id, 
                    role="ai", 
                    # The conversational AI will add the next step's question. This is the system's confirmation.
                    content=f"// System Event: Artwork '{uploaded_image.filename}' successfully uploaded and set as **artwork_image_url**. Awaiting AI's conversational response...", 
                    image_url=db_image_url
                )
                self.db.add(ai_upload_msg)
                ...
                log.error(f"Upload/Cloudinary error: {e}", exc_info=True)
        ...
        if analysis_error_message: 
            ...
            # This special string triggers the new rule in the system prompt
            final_prompt_for_llama = f"[IMAGE UPLOADED: {db_image_url}] User request: {prompt}"

        # 4. Save User Message
        # Save user prompt *without* the image URL attached, as the system/AI message now contains it.
        user_msg = ChatMessage(session_id=session_id, user_id=self.user.id, role="user", content=prompt, image_url=None) 
        self.db.add(user_msg); await self.db.flush()

        # 5. Get Context & Call LLaMA
        ...
        try:
            if intent == "general_chat":
                # If an image was just uploaded, the conversational response should be here.
                # The preview was already added in step 2.
                pass 
        ...
        # Do not save the AI response if it's the same as the system message we already added
        if db_image_url and ai_text_response in ai_upload_msg.content:
            pass
        else:
