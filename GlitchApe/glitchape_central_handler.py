...
class GlitchApeCentralHandler:
    ...
    async def _call_llama_brain(self, prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # --- PRODUCTION FIX (Resolves 502 Malformed JSON Error from logs) ---
            # Use regex to robustly extract a JSON object, even if the
            # LLM wraps it in conversational text or markdown.
            json_match = re.search(r'\{.*\}', llama_response_content, re.DOTALL)
            
            if not json_match:
                log.error(f"LLaMA failed to include a JSON object. Raw response: {llama_response_content}")
                # Provide a user-facing error that guides them, rather than a generic 502.
                # Fallback to a general_chat response
                return {
                    "intent": "general_chat",
