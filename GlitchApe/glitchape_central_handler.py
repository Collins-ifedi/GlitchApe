...
MODIFIED (Core AI Brain): Replaced LLaMA 4 (OpenRouter) with Gemini 2.5 Flash
          (Google GenAI SDK) for chat, vision, and JSON-driven intent routing.
"""
...
# --- Gemini Model (Google) Configuration (MODIFIED) ---
...
if GEMINI_API_KEY:
    try:
        # --- AUTHENTICATION FIX ---
        # Configure the SDK to use the API key explicitly.
        # This prevents it from defaulting to ADC/OAuth and failing
        # with the "API keys are not supported" error.
        genai.configure(api_key=GEMINI_API_KEY)
        
        gemini_client = genai.GenerativeModel(PRIMARY_LLM_MODEL)
        log.info(f"Gemini Client initialized for model {PRIMARY_LLM_MODEL}.")
    except Exception as e:
...
# --- MODIFIED: _analyze_image_with_vision_model (now accepts bytes) ---
...
class GlitchApeCentralHandler:
    ...
    async def _prepare_gemini_history(self, session: ChatSession, draft_details: Dict[str, Any], missing_info_str: str) -> List[Dict[str, Any]]:
        ...
        gemini_history: List[Dict[str, Any]] = [{"role": "system", "parts": [{"text": context}]}]
    ...
    # --- REPLACED: _call_llama_brain with _call_gemini_brain ---
    async def _call_gemini_brain(self, prompt_parts: List[str | genai_types.Part], history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # The history already contains memory (as a 'system' message); prompt_parts contains the new user text + image
        sdk_messages = history + [{"role": "user", "parts": prompt_parts}]

        # Configure for JSON output and inject the system prompt
        ...
        try:
            def sync_generate():
                return gemini_client.generate_content(
                    ...
                    system_instruction=GEMINI_SYSTEM_PROMPT, # Pass system prompt here
                    request_options={'timeout': GEMINI_TIMEOUT}
                )
        ...
        except google_exceptions.ResourceExhaustedError:
            ...
            raise HTTPException(502, f"AI Service API Error: {e.message[:100]}")
    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        # 5. Call LLM Brain
        action_json = await self._call_gemini_brain(gemini_prompt_parts, history)
