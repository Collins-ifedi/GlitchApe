...
MODIFIED (Local Variant Map): All Printful API catalog/variant lookups
          have been removed. The handler now loads and uses `ai_mapped_variants.json`
...
MODIFIED (Code-Side Product Validation): Implemented proactive, code-driven
          product validation. User input (e.g., "black tee") is normalized
          and checked against a pre-built catalog from `ai_mapped_variants.json`.
          If a user requests an unavailable product, color, or size, the
          code (not the AI) generates a specific error message listing
          *only* the valid options. The AI is now instructed to *only*
          extract user requests and *not* invent product options.

MODIFIED (Proactive Product Selection): The chat flow now detects the first
          user message in a new session. It intercepts the flow and forces
          the AI to present a list of valid products (from the JSON catalog)
          for the user to choose from, ensuring no invalid products are
          ever discussed.

MODIFIED (Normalization Map): Expanded the normalization map to include
          common user inputs for sizes (e.g., "large" -> "L", 
          "extra large" -> "XL").
"""
...
# We now use OpenRouter exclusively for all AI calls.
...
# --- LLaMA System Prompt (MODIFIED FOR PROACTIVE PRODUCT SELECTION) ---
LLAMA_SYSTEM_PROMPT = """
...
**NEW:** Sometimes, you will be given a `[PRODUCT_LIST: ...]`.
...
2.  **`present_product_list` (NEW)**
    * **This is a backend-forced intent.**
    * Use this *only* when the backend gives you `[PRODUCT_LIST: ...]`.
    * Your job is to welcome the user and ask them to pick *only* from that list.
    * **Example Response:**
        // Welcome to GlitchApe, cybernaut! 🚀 To fire up the design engine, first select a product from our available catalog:
        * T-Shirt
        * Hoodie
        * Long Sleeve Shirt
        * Sweatshirt
...
-   **Product Validation:** The system will validate all product, color, and size selections. Do *not* invent options or combinations. If a user asks for something unavailable (e.g., "purple polo"), just extract their request (e.g., `{"product_input": "polo", "color_input": "purple"}`). The system will automatically handle the error message.
"""
...
# This is no longer needed as we now use the variant_id from ai_mapped_variants.json
...
# This async function is no longer needed as we use the local ai_mapped_variants.json
...
async def _generate_mockup_preview(
    ...
    our local `ai_mapped_variants.json`.
    """
...
class GlitchApeCentralHandler:
    ...
    # --- NEW: Load ai_mapped_variants.json on class level ---
    @staticmethod
    @functools.lru_cache(maxsize=1)
    def _load_variant_list_from_json() -> List[Dict[str, Any]]:
        """Loads and caches the variant map from JSON."""
        try:
            # --- MODIFIED: Load the new AI-mapped file ---
            with open("ai_mapped_variants.json", "r") as f:
                data = json.load(f)
            log.info(f"Successfully loaded and cached {len(data)} variants from ai_mapped_variants.json.")
            return data
        except FileNotFoundError:
            log.error("CRITICAL: ai_mapped_variants.json not found. Product image/variant lookup will fail.")
            return []
        except Exception as e:
            log.error(f"Failed to load or parse ai_mapped_variants.json: {e}")
            return []

    # --- NEW: Build normalization map for user input ---
    @staticmethod
    @functools.lru_cache(maxsize=1)
    def _get_normalization_map() -> Dict[str, str]:
        """Returns a map of user slang to canonical product names and sizes."""
        return {
            # Products
            "tee": "T-Shirt", "t shirt": "T-Shirt", "t-shirt": "T-Shirt", "shirt": "T-Shirt",
            "sweat": "Sweatshirt", "sweat top": "Sweatshirt", "sweatshirt": "Sweatshirt",
            "hood": "Hoodie", "hoodie": "Hoodie", "hooded": "Hoodie",
            "ls tee": "Long Sleeve Shirt", "long sleeve": "Long Sleeve Shirt", "long sleeve shirt": "Long Sleeve Shirt",
            "polo": "Polo Shirt", "polo tee": "Polo Shirt", "polo shirt": "Polo Shirt",
            
            # Sizes
            "small": "S", "sml": "S",
            "medium": "M", "med": "M",
            "large": "L", "lge": "L",
            "extra large": "XL", "xtra large": "XL", "xlarge": "XL",
            "xxl": "2XL", "2xl": "2XL", "double xl": "2XL", "extra extra large": "2XL",
            "xxxl": "3XL", "3xl": "3XL", "triple xl": "3XL",
            "xxxxl": "4XL", "4xl": "4XL",
            "xxxxxl": "5XL", "5xl": "5XL",
        }

    # --- NEW: Build a fast-lookup catalog from the variant list ---
    # ----- FIX: Removed the unhashable `variant_list` argument -----
    @staticmethod
    @functools.lru_cache(maxsize=1)
    def _get_product_catalog() -> Dict[str, Dict[str, List[str]]]:
        """
        Builds a pre-processed catalog for validation by calling _load_variant_list_from_json.
        Is cached to run only once.
        Output: {
            "T-Shirt": {"colors": ["Black", "White", ...], "sizes": ["S", "M", ...]},
            "Hoodie": {"colors": ["Black", "Red", ...], "sizes": ["S", "M", ...]}
        }
        """
        # --- FIX: Call the other cached static method directly ---
        variant_list = GlitchApeCentralHandler._load_variant_list_from_json()
        if not variant_list:
            log.error("Cannot build product catalog, variant list is empty.")
            return {}

        catalog: Dict[str, Dict[str, Set[str]]] = {}
        for variant in variant_list:
            p_name = variant.get("product_name")
            if not p_name: continue
            
            if p_name not in catalog:
                catalog[p_name] = {"colors": set(), "sizes": set()}
            
            if variant.get("product_color"):
                catalog[p_name]["colors"].add(variant.get("product_color"))
            if variant.get("product_size"):
                catalog[p_name]["sizes"].add(variant.get("product_size"))
        
        # Convert sets to sorted lists for clean, non-AI-generated responses
        final_catalog: Dict[str, Dict[str, List[str]]] = {}
        for p_name, data in catalog.items():
            final_catalog[p_name] = {
                "colors": sorted(list(data["colors"])),
                "sizes": sorted(list(data["sizes"]))
            }
        
        log.info(f"Built product catalog with {len(final_catalog)} product types.")
    ...
    def __init__(self, db: AsyncSession, user: Optional[User] = None):
        ...
        # --- NEW: Build and cache product validation tools ---
        self.normalization_map = self._get_normalization_map()
        # --- FIX: Call the no-argument cached function ---
    ...
    def _get_product_image_url(self, variant_id: int) -> Optional[str]:
        ...
        log.warning(f"No product_image found in ai_mapped_variants.json for variant_id: {variant_id}")
        return None

    # --- REWRITTEN: Variant ID Lookup (Now synchronous, with code-side validation) ---
    def _validate_and_update_variant(self, draft_details: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        by searching the local `ai_mapped_variants.json` list.
        Updates draft_details with 'variant_id' and 'product' object if found.
        
        Returns (success: bool, error_message_for_user: Optional[str]).
        The error message is code-generated and lists valid options.
        """
        
        # --- 1. NORMALIZE INPUTS (from AI extraction) ---
        
        # Normalize Product Type
        raw_target_category = draft_details.get('product_input', '').lower().strip()
        # --- Use .get() with the raw key, falling back to a title-cased version ---
        target_category = self.normalization_map.get(raw_target_category, raw_target_category.title())
        if raw_target_category:
            draft_details['product_input'] = target_category # Store normalized version

        # Normalize Color
        raw_target_color = draft_details.get('color_input', '').lower().strip()
        # Find the canonical case-sensitive color name
        target_color = next((c for c in self.product_catalog.get(target_category, {}).get("colors", []) if c.lower() == raw_target_color), raw_target_color.title())
        if raw_target_color:
             draft_details['color_input'] = target_color # Store normalized version
        
        # Normalize Size
        raw_target_size = draft_details.get('size_input', '').lower().strip()
        # --- Use .get() with the raw key, falling back to an uppercased version ---
        target_size = self.normalization_map.get(raw_target_size, raw_target_size.upper())
        if raw_target_size:
            draft_details['size_input'] = target_size

        # --- 2. VALIDATE INPUTS AGAINST CATALOG ---
        
        if not raw_target_category:
            # Not enough info to try yet (user hasn't specified product)
            # This is NOT an error, just incomplete.
            return (True, None) 

        # Check if normalized product is valid
        if target_category not in self.product_catalog:
            valid_products = ", ".join(self.product_catalog.keys())
            msg = f"// Sorry, I don't recognize the product '{raw_target_category}'. I can offer: {valid_products}. What would you like?"
            return (False, msg)

        # Product is valid, check color (if provided)
        valid_colors = self.product_catalog[target_category]['colors']
        if raw_target_color and target_color not in valid_colors:
            colors_list = ", ".join(valid_colors[:10]) # Show first 10
            msg = f"// Apologies, {raw_target_color} isn't available for the {target_category}. Available colors are: {colors_list}. Which color works for you?"
            return (False, msg)
            
        # Product and color are valid, check size (if provided)
        valid_sizes = self.product_catalog[target_category]['sizes']
        if raw_target_size and target_size not in valid_sizes:
            sizes_list = ", ".join(valid_sizes)
            msg = f"// That's a no-go. {raw_target_size} isn't available for the {target_category}. Available sizes are: {sizes_list}. Which size do you need?"
            return (False, msg)

        # --- 3. CHECK FOR COMPLETENESS ---
        
        # If we have all three, we can try to find the variant
        if not all([target_category, target_size, target_color]):
            draft_details.pop('variant_id', None) # Clear old ID
            draft_details.pop('product', None) # Clear old product object
            return (True, None) # Not an error, just can't validate yet

        if not self.variant_list:
            log.error("ai_mapped_variants.json is not loaded. Cannot validate variant.")
            return (False, "// System glitch: Product catalog is offline. Please try again later.")

        # --- 4. FIND THE VARIANT ---
        
        # Search for the exact, validated, normalized matching variant
        for variant in self.variant_list:
            ...
            if p_name == target_category and size == target_size and color == target_color:
                log.info(f"Local JSON Match: Found variant {variant['variant_id']} for {target_category}, {target_size}, {target_color}")
                
                # --- NEW: Store the full object and the ID ---
                draft_details['product'] = variant
        ...
        # --- 5. FALLBACK (Combination Not Found) ---
        # This means product, color, and size are *individually* valid,
        # but the *combination* doesn't exist (e.g., "Red" Polo in "S").
        
        log.warning(f"No *combination* found in ai_mapped_variants.json for {target_category}, {target_size}, {target_color}")
        draft_details.pop('variant_id', None) # Clear any old ID
        draft_details.pop('product', None) # Clear old product object
        msg = f"// Hmm, the specific combination of {target_size} {target_color} {target_category} isn't available. Please try a different combination."
        return (False, msg)
    ...
    async def _get_memory_context(self, session: ChatSession) -> Tuple[Dict[str, Any], str]:
        ...
        if current_state == "designing":
            # --- FIX: Don't ask for product_input if we just presented the list ---
            if not draft_details.get('product_input'):
                 missing_fields.append('product_input')
            else:
                # Only ask for other fields *after* product is selected
                for key in DESIGN_STATE_FIELDS:
                    if key != 'product_input' and not draft_details.get(key):
    ...
    async def _get_chat_history_for_context(self, session: ChatSession, memory_json: str, missing_info: str, product_list: Optional[str] = None, limit: int = 10) -> List[Dict[str, str]]:
        """
        Fetches history, adds current memory, missing info, 
        and optional product list.
        """
        ...
        # --- NEW: Add product list if provided ---
        if product_list:
            context_lines.append(f"[PRODUCT_LIST: {product_list}]")
    ...
    # --- MAIN CHAT HANDLER (MODIFIED FOR PROACTIVE PRODUCT SELECTION) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        # --- ##### BEGIN BUG FIX ##### ---
        # Check for first message *before* saving the new prompt to avoid loop
        q_history = select(ChatMessage).where(ChatMessage.session_id == session_id, ChatMessage.role == 'user').limit(1)
        r_history = await self.db.execute(q_history)
        is_first_user_message_in_session = r_history.scalars().first() is None
        # --- ##### END BUG FIX ##### ---
        ...
        draft_details, missing_fields_str_before_call = await self._get_memory_context(session) # Get state *before* AI call

        # --- MODIFIED: Use the new, more accurate check ---
        # Only force the list if it's the *actual* first message AND we don't have a product.
        is_force_product_list = is_first_user_message_in_session and not draft_details.get('product_input') and session.state == "designing"
        # --- END MODIFIED CHECK ---
        ...
        # --- NEW: Proactive Product List Injection ---
        product_list_str: Optional[str] = None
        if is_force_product_list: # <--- USE NEW FLAG
            log.info(f"Session {session_id}: First message detected. Forcing product list.")
            valid_products = list(self.product_catalog.keys())
            product_list_str = ", ".join(valid_products)
            # We call the AI with the user's *original* history, but add the product list
            # and will override the AI's intent later.
            history = await self._get_chat_history_for_context(session, memory_json, missing_fields_str_before_call, product_list=product_list_str)
        else:
            # Normal history call
            history = await self._get_chat_history_for_context(session, memory_json, missing_fields_str_before_call)
        # --- END NEW LOGIC ---
        ...
        try:
            ...
            # --- NEW: Force intent if it's the first message ---
            if is_force_product_list: # <--- USE NEW FLAG
                log.info(f"Session {session_id}: Overriding AI intent to 'present_product_list'.")
                # Force a new call to the AI, this time with the *correct* history
                # that includes the user's first message, and force the intent.
                
                # Create a new system context for the *forced* call
                force_context_lines = [
                    f"[SESSION_STATE: {session.state or 'designing'}]",
                    "[MEMORY: {}]", # Empty memory
                    "[MISSING_INFO: product_input]",
                    f"[PRODUCT_LIST: {product_list_str}]"
                ]
                force_system_context = {"role": "system", "content": "\n".join(force_context_lines)}
                
                # Create a *new* history for this one-time call
                # We *only* want the system prompt and our new forced context.
                # We *don't* include the user's message, as the AI's job is to *ask*, not reply.
                force_history = [force_system_context]
                
                # We need to tell the AI what its intent is. We do this by adding a
                # fake "user" message that just contains the intent. This is a bit
                # of a hack, but it's the simplest way to guide the AI.
                # A cleaner way: just build the response ourselves.
                
                # --- BETTER FIX: Just build the response ourselves. ---
                valid_products = list(self.product_catalog.keys())
                products_list_md = "\n* ".join(valid_products)
                ai_text_response = f"""// Welcome to GlitchApe, cybernaut! 🚀 To fire up the design engine, first select a product from our available catalog:
* {products_list_md}

What are we creating today?"""
                action_json = {"intent": "present_product_list"}
                
                # --- We must also update the draft_details with the user's *first* message ---
                # The AI *extracted* this, but we overrode it. We must re-run extraction.
                # This is tricky. Let's just let the *next* message handle it.
                # For now, we clear any memory the AI *might* have hallucinated.
                draft_details = {} 
                # --- END BETTER FIX ---
        ...
        try:
            # --- NEW: Handle the forced product list intent ---
            if intent == "present_product_list":
                # The ai_text_response is already set (either by AI or by our code override)
                pass
            ...
            # --- REMOVED: 'image_analysis_confirm' intent ---
                ...
                # --- FIX: Check if *any* variant key was updated OR if the variant_id is *still* missing ---
                is_missing_variant_id = not draft_details.get('variant_id')
                if variant_keys_updated or (is_missing_variant_id and session.state == "designing"):
                # --- END FIX ---
                
                    # --- MODIFIED: Call is now synchronous and provides rich errors ---
                    ...
                    if not validation_ok:
                        ...
                        # --- MODIFIED: Use product object from draft ---
                        if draft_details.get('product'):
                    ...
                    # --- FIX: Only show image if validation passed AND it's a *new* ID ---
                    if validation_ok and new_variant_id and draft_details.get('product'): # 'product' is the flag for success
                    # --- END FIX ---
                        ...
                        if product_image_url:
                            ...
                            # Only add if we are in designing state (to prevent repetition)
                            if session.state == "designing":
            ...
            # --- NEW: Handle explicit review from AI ---
                ...
                if session.state != "designing":
                    ...
                    # --- FIX: Final check before moving state ---
                    # We must re-fetch the missing fields *after* all logic has run
                    temp_drafts, missing_fields_after_call = await self._get_memory_context(session)
                    if "variant_id" in missing_fields_after_call or "product_input" in missing_fields_after_call or "size_input" in missing_fields_after_call or "color_input" in missing_fields_after_call:
                        ai_text_response = f"// Hold up! I'm still missing some product details before we can move on. I still need: {missing_fields_after_call}"
                        validation_clarification = ai_text_response # Override AI response
                    else:
            ...
            # --- NEW: Handle explicit checkout confirmation from AI ---
                ...
                if session.state != "awaiting_confirmation":
                    ...
                    if await self._check_all_fields_collected(draft_details):
                        ...
                        # --- FIX: Get the *actual* missing fields to tell the user ---
                        _, missing_fields_final = await self._get_memory_context(session)
                        log.error(f"Missing fields: {missing_fields_final}")
                        ai_text_response = f"// System integrity fault! Your order details are incomplete. I'm still missing: {missing_fields_final}. Reverting to review."
                        # --- END FIX ---
    ...
    async def initiate_checkout(self, req: CheckoutRequest) -> CheckoutResponse:
        ...
        try:
            # --- MODIFIED: Pull from 'product' object ---
            product_obj = draft_details.get('product')
            if not product_obj or not isinstance(product_obj, dict):
                raise ValueError("Missing or invalid 'product' object in draft.")
                
            variant_id = product_obj.get('variant_id')
            ...
            prod_name = product_obj.get('product_name')
            if not prod_name:
