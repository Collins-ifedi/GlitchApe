...
class GlitchApeCentralHandler:
    ...
    # --- MODIFIED: Renamed and updated for Gemini SDK ---
    async def _get_gemini_history(self, session: ChatSession, memory_json: str, missing_info: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Fetches history, formats for Gemini SDK, adds memory context."""
        ...
        # This first 'user' message provides the dynamic context. 
        # The 'model' response ensures the chat history alternates correctly.
        gemini_history = [
            {"role": "user", "parts": [{"text": "\n".join(context_lines)}]},
            {"role": "model", "parts": [{"text": "OK, I have the latest memory context. Ready for the user's request."}]}
        ]
    ...
    async def _call_gemini_brain(self, prompt_parts: List[str | genai_types.Part], history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # The history already contains memory; prompt_parts contains the new user text + image
        ...
        try:
            ...
            if not json_match:
                ...
                raise HTTPException(502, "AI failed to produce valid JSON intent.")
            ...
            try:
                ...
                log.error(f"Gemini invalid JSON detected. Err: {e}. Str: {json_part}. Full Raw: {gemini_response_content}")
                raise HTTPException(502, f"AI returned malformed JSON. Please try again.")

            return action_json

        except google_exceptions.GoogleAPIError as e:
            log.error(f"Gemini API Error: {e}", exc_info=True)
            # Check for the specific auth error we discussed
            if "API keys are not supported" in str(e):
                log.critical("FATAL: Gemini auth error. The SDK is using ADC/OAuth. Check server env vars for GOOGLE_GENAI_USE_VERTEXAI or GOOGLE_APPLICATION_CREDENTIALS.")
                raise HTTPException(502, f"AI Auth Error. Check server logs.")
            raise HTTPException(502, f"AI Service Error: {e.message[:100]}")
        except Exception as e:
            log.error(f"Unhandled error in _call_gemini_brain: {e}", exc_info=True)
            raise HTTPException(500, detail=f"Internal error during AI processing: {str(e)[:100]}")
    ...
    # --- MAIN CHAT HANDLER (MODIFIED FOR GEMINI & MULTIMODALITY) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        # 2. Handle Upload & Vision (This updates memory *before* LLM call)
        # --- MODIFIED: This section now prepares multimodal input ---
        gemini_prompt_parts: List[str | genai_types.Part] = []
        ...
        if uploaded_image and uploaded_image.filename:
            try:
                ...
                # --- NEW: Fetch bytes ONCE from Cloudinary ---
                # This implements the "fetch from URL" pattern from the docs
                ...
                image_part = genai_types.Part.from_bytes(data=image_bytes_for_analysis, mime_type=mime_type_for_analysis)
                gemini_prompt_parts.append(image_part)
        ...
        # 3. Prepare Final Gemini Prompt
        # Add the user's text *after* the image, as recommended by docs
        
        final_text_prompt = prompt
        if image_analysis_text: final_text_prompt = f"[IMAGE ANALYSIS: {image_analysis_text}] User request: {prompt}"
        elif analysis_error_message: final_text_prompt = f"[IMAGE ANALYSIS FAILED: {analysis_error_message}] User request: {prompt}"
        elif db_image_url: final_text_prompt = f"[IMAGE UPLOADED: {db_image_url}] User request: {prompt}" # URL is now Cloudinary

        gemini_prompt_parts.append(final_text_prompt)


        # 4. Save User Message
        user_msg = ChatMessage(session_id=session_id, user_id=self.user.id, role="user", content=prompt, image_url=db_image_url) # Save upload URL
        self.db.add(user_msg); await self.db.flush()

        # 5. Get Context & Call Gemini
        memory_json = json.dumps(draft_details)
        history = await self._get_gemini_history(session, memory_json, missing_fields) # <-- RENAMED
        try:
            # Pass the multimodal prompt parts
            action_json = await self._call_gemini_brain(gemini_prompt_parts, history) # <-- RENAMED
        except HTTPException as e:
            ai_text_response = f"My core processor (Gemini) had an issue ({e.detail}). Please try again." # <-- RENAMED
        ...
        # This flat if/elif block replaces the old rigid 'current_state' logic.
        # We trust the LLM's intent, regardless of the session's state.
        ...
        try:
            if intent == "general_chat":
                pass # No state or memory change
                ...
                # The artwork_image_url was already set in Step 2.
                # We just add the preview.
                if 'artwork_image_url' in draft_details:
