...
# --- Gemini Model (Google) Configuration (Unified for Text and Vision) ---
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
if not GEMINI_API_KEY: log.warning("GEMINI_API_KEY not set. Vision features disabled.")
VISION_MODEL = "gemini-2.5-flash" # This model will be used for both text and vision
...
# This prompt is now used as the 'system_instruction' for Gemini
LLAMA_SYSTEM_PROMPT = """
...
class GlitchApeCentralHandler:
    ...
    async def _get_chat_history_for_context(self, session: ChatSession, memory_json: str, missing_info: str, limit: int = 10) -> List[Dict[str, str]]:
        ...
        gemini_history = [{"role": "system", "content": "\n".join(context_lines)}]
        # --- END MODIFICATION ---
        ...
        for msg in reversed(messages):
            ...
            gemini_history.append({"role": api_role, "content": content})
            # --- END FIX ---
            
        return gemini_history

    async def _call_gemini_brain(self, prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        """Calls Gemini via SDK, handles JSON extraction and errors, and enforces JSON output."""
        global gemini_client
        if not gemini_client:
            raise HTTPException(503, "Gemini Client not initialized. Check API Key.")

        # Convert the simplified history to the format expected by the Gemini SDK
        # SDK uses 'model' for the AI role and requires 'parts' for content
        sdk_messages = []
        system_prompt_main = LLAMA_SYSTEM_PROMPT # Use the existing prompt
        
        # The history list from _get_chat_history_for_context includes the dynamic memory
        # as the first "system" message. We need to extract it.
        system_prompt_dynamic = ""
        if history and history[0]["role"] == "system":
            system_prompt_dynamic = history[0]['content']
            history = history[1:] # Remove the system message from the main history
        
        # Combine them
        full_system_prompt = f"{system_prompt_main}\n\n{system_prompt_dynamic}"
        
        for msg in history:
            role = "model" if msg["role"] == "assistant" else msg["role"]
            sdk_messages.append({"role": role, "parts": [{"text": msg["content"]}]})

        # Add the current user prompt
        sdk_messages.append({"role": "user", "parts": [{"text": prompt}]})
        ...
        # Use the 'system_instruction' parameter for the main system prompt
        model_with_system_prompt = genai.GenerativeModel(
            VISION_MODEL, # Use the same model
            system_instruction=full_system_prompt
        )

        try:
            def sync_generate():
                return model_with_system_prompt.generate_content(
                    sdk_messages,
            ...
            # --- PRODUCTION FIX (Robust JSON extraction) ---
            # Use regex to robustly extract a JSON object, even if the
            # model *somehow* adds markdown fences.
            json_match = re.search(r'\{.*\}', gemini_response_content, re.DOTALL)
            
            if not json_match:
                log.error(f"Gemini failed to include a JSON object. Raw response: {gemini_response_content}")
                raise HTTPException(502, "The AI failed to return a valid action. Please try rephrasing.")

            json_str = json_match.group(0)
            try:
                return json.loads(json_str)
            except json.JSONDecodeError as e:
                log.error(f"Gemini invalid JSON detected. Err: {e}. Str: {json_str}. Full Raw: {gemini_response_content}")
    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        try:
            action_json = await self._call_gemini_brain(final_prompt_for_llama, history)
