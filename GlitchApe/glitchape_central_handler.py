...
4.  **`image_analysis_confirm`**
    * Use this *only* after the system has analyzed a user-uploaded image (`[IMAGE ANALYSIS: ...]`) or one was just uploaded (`[IMAGE UPLOADED: ...]`).
    * Your job is to:
    * 1. Acknowledge the image: "Got the visual data! Looks like [Summary]."
    * 2. Ask to confirm its use: "Ready to use this for the design?"
    * 3. Ask for the *next logical missing info* to place it. Check [MISSING_INFO].
    * 4. Good first questions are **product_input** (e.g., 't-shirt') and **placement_id** (e.g., 'front', 'back', 'sleeve').
    * 5. After that, you MUST ask for **placement_position** (e.g., 'How many **inches wide** and **inches down from the collar**?').
    * Action: `{"intent": "image_analysis_confirm", "response_text": "Got the visual data! Looks like [Summary]. Ready to place this on a product? If so, what **product** (t-shirt, hoodie) and **placement** (front, back) are you thinking?", "analysis_prompt": "Uploaded image analysis provided."}`
...
class GlitchApeCentralHandler:
    ...
    async def _get_chat_history_for_context(self, session: ChatSession, memory_json: str, missing_info: str, limit: int = 10) -> List[Dict[str, str]]:
        ...
        for msg in reversed(messages):
            ...
            # --- MODIFIED: Safety wrapper for user messages in history ---
            if api_role == "user":
                content = f"User said: '{content}'. Respond helpfully, safely, and constructively."
    ...
    async def _call_llama_brain(self, prompt: str, history: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        # --- MODIFIED: Safety wrapper for the current user prompt ---
        safe_prompt = f"User said: '{prompt}'. Respond helpfully, safely, and constructively."
        
        messages = [ {"role": "system", "content": LLAMA_SYSTEM_PROMPT}, *history, {"role": "user", "content": safe_prompt} ]
        ...
        try:
            ...
            if not json_match:
                ...
                raise HTTPException(502, "The AI failed to return a valid action. Please try rephrasing your request.")
