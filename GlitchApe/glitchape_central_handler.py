...
class GlitchApeCentralHandler:
    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str) -> AIResponse:
        ...
        new_state = "designing" # Default to the main state
        ...
        try:
            ...
            if action == "generate_design":
                ...
                # Keep the state the same unless Llama implies a design is being worked on
                new_state = session.state or "designing" 
...
async def get_image(filename: str):
    ...
    if not os.path.exists(file_path) or ".." in filename or filename.startswith("/"):
        raise HTTPException(status_code=44, detail="Image not found")
