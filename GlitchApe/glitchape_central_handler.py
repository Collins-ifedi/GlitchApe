    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str) -> AIResponse:
        ...
        try:
            ...
            if action == "generate_design":
                ...
                # --- FIX: The state (new_state) is already set to the session's current state, so no change is needed. ---
                # The old buggy line 'new_state = session.state or "designing"' is removed.
