...
class GlitchApeCentralHandler:
    ...
    def _validate_and_update_variant(self, draft_details: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        for variant in self.variant_list:
            ...
            # --- MODIFIED: Match `product_input` (e.g. "t-shirt") against `categories` ("t-shirts")
            category_match = (target_category in category)
            
            # --- MODIFIED: Check against the new simplified product name (e.g. "T-Shirt(S, White)")
            # This allows the AI to use the simple name from the file
            ai_mapped_name_match = (target_category.lower() in product_name.lower())
            
            if (category_match or ai_mapped_name_match) and target_size == size and target_color == color:
        ...
        log.warning(f"No match found in ai_mapped_variants.json for {target_category}, {target_size}, {target_color}")
