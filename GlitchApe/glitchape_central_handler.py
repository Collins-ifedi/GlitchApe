...
# FIX: REMOVE the specific import below, as it causes the "cannot import name 'ResourceExhaustedError'" error
    ...
    try:
        ...
        def sync_generate():
            return gemini_client.models.generate_content(
                ...
                # REMOVED: request_options={'timeout': GEMINI_TIMEOUT} <-- FIX
            )
...
class GlitchApeCentralHandler:
    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        history = await self._prepare_gemini_history(session, draft_details, missing_info_str) 
        
        # 5. Call LLM Brain (REFACTORED: Pass context here)
        action_json = await self._call_gemini_brain(
            gemini_prompt_parts, 
            history,
            draft_details,
            missing_info_str
...
async def start_new_chat_session(
    ...
    try:
        ...
        # --- FIX: Flush the session to get the new_session.id populated ---
        await db.flush() 
        # --- End of FIX ---
...
@router.post("/ai/chat") # <-- FIX 1: Changed from "/chat/{session_id}"
async def send_chat_message_endpoint(
    # session_id: uuid.UUID, # <-- Removed from path
    session_id: str = Form(...),      # <-- FIX 2: Get session_id from Form data
    prompt: str = Form(...),
    ...
    # --- FIX 3: Convert the string session_id to a UUID ---
    try:
    ...
    return await handler.handle_chat_message(session_uuid, prompt, uploaded_image) # Pass the UUID
