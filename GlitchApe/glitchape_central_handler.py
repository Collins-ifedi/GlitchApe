...
MODIFIED (Checkout Flow): AI is now instructed to summarize the order and
          direct the user to click the "Checkout" button once all
          [MISSING_INFO] is collected. The `checkout_ready` state is set
          by the handler when all data is present.
"""
...
LLAMA_MODEL = "meta-llama/llama-4-maverick:free" # Use this model for all requests
...
# --- LLaMA System Prompt (MODIFIED FOR UI CHECKOUT FLOW) ---
LLAMA_SYSTEM_PROMPT = """
...
1.  **`general_chat`**
    ...
    * Use this to respond if the user asks to upload an image.
    * **Example Response:**
...
2.  **`design_request`**
    ...
    * This is the **ONLY** way to create artwork.
    * **Example Response:**
...
3.  **`design_revision_artwork`**
    ...
    * **Example Response:**
        Recalibrating pixels... Change incoming! Let's see how this looks in neon green.
        
        ```json
        {
          "intent": "design_revision_artwork",
          "revision_prompt": "make it neon green"
        }
        ```
...
-   **FINAL STEP (CRITICAL):** When `[MISSING_INFO: ...]` is empty, your natural language response MUST be a summary of all data in `[MEMORY: ...]`. This summary MUST end with the exact instruction: "If everything looks correct, click the **Checkout** button at the top to proceed with payment." The JSON intent for this summary message MUST be `general_chat`.
...
class GlitchApeCentralHandler:
    ...
    async def _get_memory_context(self, session: ChatSession) -> Tuple[Dict[str, Any], str]:
        ...
        # --- Only add fields as "missing" if the state is NOT checkout_ready ---
    ...
    # --- MAIN CHAT HANDLER (MODIFIED FOR MIXED-MODE RESPONSE) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        # --- REMOVED: `ai_text_response = action_json.get(...)` ---
        # ai_text_response is now the natural language part from the AI.
        ...
        try:
            # --- MODIFIED: This function now *silently* sets the state
            # The AI is responsible for the summary message via its prompt.
            if await self._check_and_set_checkout_ready(session, draft_details):
                # We no longer append "All data compiled!" here.
                pass
    ...
    async def initiate_checkout(self, req: CheckoutRequest) -> CheckoutResponse:
        ...
        if session.state != "ready_for_checkout": 
            ...
            raise HTTPException(400, "Almost ready! Please complete all the order details in the chat first.")
