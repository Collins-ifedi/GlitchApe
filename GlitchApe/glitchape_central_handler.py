...
from pydantic import BaseModel # Corrected from pantic
...
LLAMA_MODEL = "meta-llama/llama-4-maverick:free" # Using LLaMA 4 Maverick as requested
...
**RULES:**
-   **You MUST include a single, valid JSON object in your response, which should be the LAST thing you output.**
    ...
    try:
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                # Optionally, force load to catch more issues, but slower: img.load()
...
class GlitchApeCentralHandler:
    ...
    async def _get_chat_history_for_context(self, session_id: uuid.UUID, limit: int = 10) -> List[Dict[str, str]]:
        ...
        for msg in reversed(messages):
            # CRITICAL FIX: Map local role 'ai' to API standard 'assistant'
            api_role = msg.role
            if api_role == "ai":
                api_role = "assistant"
            
            # Ensure only 'role' and 'content' are included for the API
            llama_history.append({"role": api_role, "content": msg.content})
    ...
    async def _call_llama_brain(
        ...
        try:
            ...
            if start != -1 and end != -1 and end > start:
                ...
                # If no clear JSON object found, the model failed to follow the key rule.
                log.error(f"LLaMA failed to include a recognizable JSON object. Raw response: {llama_response_content}")
                # As a final attempt, just try to parse the whole response.
    ...
    async def initiate_checkout(self, req: CheckoutRequest) -> CheckoutResponse:
        ...
        # --- NEW: Validate checkout request data ---
        # 1. Check for critical recipient data (Printful requirement)
        ...
        if missing_fields:
            raise HTTPException(
                status_code=400,
                detail=f"Missing required recipient information: {', '.join(missing_fields)}."
            )

        # 2. Check for minimum order quantity
        if req.item.quantity <= 0:
            raise HTTPException(
                status_code=400,
                detail="Order quantity must be a positive number."
            )
        # --- END OF NEW ---
    ...
    async def handle_image_placement(
        ...
        # --- NEW: Basic Filename Hygiene Check ---
        def is_safe_filename(name):
            return name and not (".." in name or "/" in name or "\\" in name)

        if not is_safe_filename(base_filename) or not is_safe_filename(overlay_filename):
            # 400 is more appropriate than 404 since the request itself is malformed/unsafe
            raise HTTPException(status_code=400, detail="Invalid or unsafe filename provided.")
        # --- END OF NEW ---
        ...
        try:
            ...
            # Re-raise HTTPException (e.g., from _place_uploaded_image if files were invalid PIL input)
            raise e
        except Exception as e:
            log.error(f"Error placing image: {e}")
            raise HTTPException(status_code=500, detail=f"Error processing image placement: {e}")
