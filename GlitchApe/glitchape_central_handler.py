...
# --- LLaMA System Prompt (MODIFIED FOR AI-ONLY FLOW) ---
LLAMA_SYSTEM_PROMPT = """
...
-   **Artwork Creation:** The *only* way to create an `artwork_image_url` is with the `design_request` intent.
...
class GlitchApeCentralHandler:
    ...
    # --- MODIFIED: _call_openrouter_brain (Multimodal payload removed) ---
    async def _call_openrouter_brain(self, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    # --- MAIN CHAT HANDLER (MODIFIED TO REJECT UPLOADS) ---
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        try:
            # --- MODIFIED: Call (no multimodal_payload) ---
            action_json = await self._call_openrouter_brain(history)
