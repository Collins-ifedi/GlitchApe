...
class GlitchApeCentralHandler:
    ...
    @staticmethod
    @functools.lru_cache(maxsize=1)
    def _get_product_catalog(variant_list: Tuple[Dict[str, Any], ...]) -> Dict[str, Dict[str, List[str]]]:
        """
        Builds a pre-processed catalog for validation.
        Input: Tuple of variant dicts (must be hashable for lru_cache).
        Output: {
    ...
    def __init__(self, db: AsyncSession, user: Optional[User] = None):
        ...
        # Pass a tuple to the cacheable function
        self.product_catalog = self._get_product_catalog(tuple(self.variant_list))
