...
# --- LLaMA System Prompt (MODIFIED FOR STATE-AWARE FLOW & PRODUCT VALIDATION) ---
LLAMA_SYSTEM_PROMPT = """
...
class GlitchApeCentralHandler:
    ...
    def _get_normalization_map() -> Dict[str, str]:
        """Returns a map of user slang to canonical product names."""
        return {
    ...
    def _validate_and_update_variant(self, draft_details: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        # Normalize Size
        target_size = draft_details.get('size_input', '').upper().strip() # Sizes are UPPERCASED in JSON
        if target_size:
        ...
        if target_size and target_size not in valid_sizes:
            sizes_list = ", ".join(valid_sizes)
            msg = f"// That's a no-go. {target_size} isn't available for the {target_category}. Available sizes are: {sizes_list}. Which size do you need?"
            return (False, msg)
