    ...
    async def handle_chat_message(self, session_id: uuid.UUID, prompt: str, uploaded_image: Optional[UploadFile] = None) -> LiveChatResponse:
        ...
        if uploaded_image and uploaded_image.filename:
            try:
                ...
                # --- REMOVED: Vision analysis is no longer performed ---
        ...
        try:
            if intent == "general_chat":
                # This is now also triggered by the new [IMAGE UPLOADED] rule.
                # We also add the preview image if it was just uploaded.
                if db_image_url:
                    image_previews.append(ImagePreview(view_name="Uploaded Artwork", url=db_image_url))
